...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  filter(str_detect(Kdinacbgs,c("O")))
...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmppklayan,Tgldtgsjp,
          Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,biayars,Biayaverifikasi) %>%
  filter(str_detect(Kdinacbgs,c("Z"))) %>%
  filter(str_detect(Procedure,c("ultrasound")))
...
datausg$keterangan <- "USG"

datausg$Tgldtgsjp <- as.Date(datausg$Tgldtgsjp, format = "%m/%d/%Y")
datausg$Tglplgsjp <- as.Date(datausg$Tglplgsjp, format = "%m/%d/%Y")

#mencari pasien persalinan
datanokapst <- data %>%
  dplyr::group_by(Nokapst)
datanokapst <- datanokapst %>% select(Nokapst)

#mencari pasien usg karena persalinan
datausgpersalinan <- left_join(datanokapst, datausg, by = "Nokapst")
