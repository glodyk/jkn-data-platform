...
data19 <- read_csv("Sheet 1_Full Data gresik 2019.csv") %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Kdppklayan,Nosjp,Nmtkp,
          ...
          Namadpjp,Nmjnspulang,Flagspesialistik,biayars,Biayaverifikasi) %>%
  subset(Nmtkp == "RJTL") %>%
...
kepwil <- left_join(falseEMG, data, by = c("Nosjp"="Nosjp"))
kepwil$Keterangan <- "Dari Kepwil"

newdata <- data %>%
  filter(!str_detect(Nmjnspulang,c("Meninggal"))) %>%
  subset(is.na(Diagsekunder)) %>%
  subset(is.na(Procedure)) %>%
  mutate(Diagnosa = paste0(as.character(Kddiagprimer)," - ",
                           as.character(Nmdiagprimer))) %>%
  filter(!str_detect(Diagnosa,c("E86|Dyspnoea|Fever|fever|titis|Fracture|
                                fracture|colic|Colic|Chest pain|chest pain|
                                Epistaxis|Volume depletion|plegia|enteritis|
                                J45|Syncope|wound|Haemoptysis|
                                intoxication|oxic")))

newdata1 <- left_join(New_DF,newdata, by = c("Diagnosa"="Diagnosa")) %>% drop_na(Nosjp)
newdata1 <- newdata1 %>% subset(Tglpelayanan >= "2021-01-01")
newdata1$Keterangan <- "Tambahan KC Gresik"
newdata1$Diagnosa <- NULL

audit <- rbind(newdata1,kepwil)
rm(newdata1,kepwil)

audit <- audit[order(audit$Nokapst, audit$Tglplgsjp),]

audit <- audit %>% 
  arrange(Nosjp, Keterangan) %>%
  distinct(Nosjp, .keep_all = TRUE)

cek <- audit %>% subset(Keterangan == "Dari Kepwil")
...
audit <- audit %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nosjp,Nmtkp,
          ...
          Namadpjp,Nmjnspulang,Flagspesialistik,biayars,Biayaverifikasi,
          Keterangan)
write.xlsx(audit, file = "1.Potensi False emergency.xlsx")
#========================================================================
