...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  ...
  filter(str_detect(Procedure,c("9851|9852|9859")))
...
data <- data %>% group_by(Nokapst) %>%
  mutate(Keterangan = paste0("ESWL ke-", sequence(n())))
...
write.csv(data, "D:/data gresik/MTF KC GRESIK/gresik_2020/db ESWL.csv",
          na="", row.names = FALSE)
