...
stroke <- data %>%
  ...
  filter(str_detect(Diagnosa,"I60|I61|I62|I63|I64|I65|I66|I67|I68|I69")) %>%
  select(-Nmtkp)

stroke$Keterangan <- with(stroke,
                          ifelse(Kdinacbgs == "M-3-16-0","Fisioterapi",
                                 ifelse(Kdinacbgs == "Z-3-12-0","Fisioterapi",Sumber)))
#========================================================================
stroke <- stroke %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
stroke <- stroke %>% mutate(Diagsekunder = na_if(Diagsekunder, ""))
stroke <- stroke %>% mutate(Procedure = na_if(Procedure, "-"))
stroke <- stroke %>% mutate(Procedure = na_if(Procedure, ""))
...
stroke$Nmdiagprimer1 <- with(stroke,
                             ifelse(is.na(stroke$Kddiagprimer1),NA,Nmdiagprimer))
stroke <- stroke %>%
  mutate(Diagprimer = paste0(as.character(Kddiagprimer1)," - ",
                             as.character(Nmdiagprimer1)))
stroke$Diagprimer <- as.character(trimws(gsub("NA - NA",NA,stroke$Diagprimer)), "both")
stroke <- stroke %>%
  mutate(Diagnosis = paste0(as.character(Diagprimer),";",
                           as.character(Diagsekunder)))
stroke$Diagnosis <- as.character(trimws(gsub(";NA|NA;","",stroke$Diagnosis)), "both")
stroke <- stroke %>% select(-Kddiagprimer1,-Nmdiagprimer1,-Diagprimer)

stroke <- stroke %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
stroke <- stroke %>% mutate(Procedure = na_if(Procedure, "-"))
stroke$Diagnosa <- as.character(trimws(stroke$Diagnosa), "both")
stroke$Diagnosis <- as.character(trimws(stroke$Diagnosis), "both")

library(splitstackshape)
stroke1 <- concat.split(stroke, "Diagnosis", ";")
colnames(stroke1)

stroke1$Diagnosis_1 <- as.character(trimws(stroke1$Diagnosis_1))
stroke1$Diagnosis_2 <- as.character(trimws(stroke1$Diagnosis_2))
stroke1$Diagnosis_3 <- as.character(trimws(stroke1$Diagnosis_3))
stroke1$Diagnosis_4 <- as.character(trimws(stroke1$Diagnosis_4))
stroke1$Diagnosis_5 <- as.character(trimws(stroke1$Diagnosis_5))
stroke1$Diagnosis_6 <- as.character(trimws(stroke1$Diagnosis_6))
stroke1$Diagnosis_7 <- as.character(trimws(stroke1$Diagnosis_7))
stroke1$Diagnosis_8 <- as.character(trimws(stroke1$Diagnosis_8))
stroke1$Diagnosis_9 <- as.character(trimws(stroke1$Diagnosis_9))

stroke1$Diagnosis_1 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_1),NA,Diagnosis_1))
stroke1$Diagnosis_2 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_2),NA,Diagnosis_2))
stroke1$Diagnosis_3 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_3),NA,Diagnosis_3))
stroke1$Diagnosis_4 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_4),NA,Diagnosis_4))
stroke1$Diagnosis_5 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_5),NA,Diagnosis_5))
stroke1$Diagnosis_6 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_6),NA,Diagnosis_6))
stroke1$Diagnosis_7 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_7),NA,Diagnosis_7))
stroke1$Diagnosis_8 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_8),NA,Diagnosis_8))
stroke1$Diagnosis_9 <- with(stroke1,ifelse(is.na(stroke1$Diagnosis_9),NA,Diagnosis_9))
...
stroke1 <- stroke1 %>%
  mutate(
    Diagnosis_08 = case_when(
      Diagnosis_8 == "Z014 - Gynaecological examination (general)(routine)" ~ NA,
      Diagnosis_8 == "Z016 - Radiological examination, not elsewhere classified" ~ NA,
      Diagnosis_8 == "Z030 - Observation for suspected tuberculosis" ~ NA,
      Diagnosis_8 == "Z031 - Observation for suspected malignant neoplasm" ~ NA,
      Diagnosis_8 == "Z038 - Observation for other suspected diseases and conditions" ~ NA,
      Diagnosis_8 == "Z039 - Observation for suspected disease or condition, unspecified" ~ NA,
      Diagnosis_8 == "Z041 - Examination and observation following transport accident" ~ NA,
      Diagnosis_8 == "Z048 - Examination and observation for other specified reasons" ~ NA,
      Diagnosis_8 == "Z080 - Follow-up examination after surgery for malignant neoplasm" ~ NA,
      Diagnosis_8 == "Z088 - Follow-up examination after other treatment for malignant neoplasm" ~ NA,
      Diagnosis_8 == "Z089 - Follow-up examination after unspecified treatment for malignant neoplasm" ~ NA,
      Diagnosis_8 == "Z090 - Follow-up examination after surgery for other conditions" ~ NA,
      Diagnosis_8 == "Z091 - Follow-up examination after radiotherapy for other conditions" ~ NA,
      Diagnosis_8 == "Z092 - Follow-up examination after chemotherapy for other conditions" ~ NA,
      Diagnosis_8 == "Z093 - Follow-up examination after psychotherapy" ~ NA,
      Diagnosis_8 == "Z094 - Follow-up examination after treatment of fracture" ~ NA,
      Diagnosis_8 == "Z097 - Follow-up examination after combined treatment for other conditions" ~ NA,
      Diagnosis_8 == "Z098 - Follow-up examination after other treatment for other conditions" ~ NA,
      Diagnosis_8 == "Z099 - Follow-up examination after unspecified treatment for other conditions" ~ NA,
      Diagnosis_8 == "Z209 - Contact with and exposure to unspecified communicable disease" ~ NA,
      Diagnosis_8 == "Z291 - Prophylactic immunotherapy" ~ NA,
      Diagnosis_8 == "Z301 - Insertion of (intrauterine) contraceptive device" ~ NA,
      Diagnosis_8 == "Z340 - Supervision of normal first pregnancy" ~ NA,
      Diagnosis_8 == "Z348 - Supervision of other normal pregnancy" ~ NA,
      Diagnosis_8 == "Z349 - Supervision of normal pregnancy, unspecified" ~ NA,
      Diagnosis_8 == "Z351 - Supervision of pregnancy with history of abortive outcome" ~ NA,
      Diagnosis_8 == "Z358 - Supervision of other high-risk pregnancies" ~ NA,
      Diagnosis_8 == "Z359 - Supervision of high-risk pregnancy, unspecified" ~ NA,
      Diagnosis_8 == "Z370 - Single live birth" ~ NA,
      Diagnosis_8 == "Z390 - Care and examination immediately after delivery" ~ NA,
      Diagnosis_8 == "Z392 - Routine postpartum follow-up" ~ NA,
      Diagnosis_8 == "Z419 - Procedure for purposes other than remedying health state, unspecified" ~ NA,
      Diagnosis_8 == "Z429 - Follow-up care involving plastic surgery, unspecified" ~ NA,
      Diagnosis_8 == "Z470 - Follow-up care involving removal of fracture plate and other internal fixation device" ~ NA,
      Diagnosis_8 == "Z478 - Other specified orthopaedic follow-up care" ~ NA,
      Diagnosis_8 == "Z479 - Orthopaedic follow-up care, unspecified" ~ NA,
      Diagnosis_8 == "Z480 - Attention to surgical dressings and sutures" ~ NA,
      Diagnosis_8 == "Z488 - Other specified surgical follow-up care" ~ NA,
      Diagnosis_8 == "Z489 - Surgical follow-up care, unspecified" ~ NA,
      Diagnosis_8 == "Z490 - Preparatory care for dialysis" ~ NA,
      Diagnosis_8 == "Z491 - Extracorporeal dialysis" ~ NA,
      Diagnosis_8 == "Z492 - Other dialysis" ~ NA,
      Diagnosis_8 == "Z500 - Cardiac rehabilitation" ~ NA,
      Diagnosis_8 == "Z501 - Other physical therapy" ~ NA,
      Diagnosis_8 == "Z502 - Alcohol rehabilitation" ~ NA,
      Diagnosis_8 == "Z503 - Drug rehabilitation" ~ NA,
      Diagnosis_8 == "Z504 - Psychotherapy, not elsewhere classified" ~ NA,
      Diagnosis_8 == "Z505 - Speech therapy" ~ NA,
      Diagnosis_8 == "Z506 - Orthoptic training" ~ NA,
      Diagnosis_8 == "Z507 - Occupational therapy and vocational rehabilitation, not elsewhere classified" ~ NA,
      Diagnosis_8 == "Z508 - Care involving use of other rehabilitation procedures" ~ NA,
      Diagnosis_8 == "Z509 - Care involving use of rehabilitation procedure, unspecified" ~ NA,
      Diagnosis_8 == "Z510 - Radiotherapy session" ~ NA,
      Diagnosis_8 == "Z511 - Chemotherapy session for neoplasm" ~ NA,
      Diagnosis_8 == "Z512 - Other chemotherapy" ~ NA,
      Diagnosis_8 == "Z513 - Blood transfusion (without reported diagnosis)" ~ NA,
      Diagnosis_8 == "Z514 - Preparatory care for subsequent treatment, not elsewhere classified" ~ NA,
      Diagnosis_8 == "Z515 - Palliative care" ~ NA,
      Diagnosis_8 == "Z516 - Desensitization to allergens" ~ NA,
      Diagnosis_8 == "Z518 - Other specified medical care" ~ NA,
      Diagnosis_8 == "Z519 - Medical care, unspecified" ~ NA,
      Diagnosis_8 == "Z549 - Convalescence following unspecified treatment" ~ NA,
      Diagnosis_8 == "Z590 - Homelessness" ~ NA,
      Diagnosis_8 == "Z596 - Low income" ~ NA,
      Diagnosis_8 == "Z719 - Counselling, unspecified" ~ NA,
      Diagnosis_8 == "Z760 - Issue of repeat prescription" ~ NA,
      Diagnosis_8 == "Z801 - Family history of malignant neoplasm of trachea, bronchus and lung" ~ NA,
      Diagnosis_8 == "Z898 - Acquired absence of upper and lower limbs [any level]" ~ NA,
      Diagnosis_8 == "Z908 - Acquired absence of other organs" ~ NA,
      Diagnosis_8 == "Z941 - Heart transplant status" ~ NA,
      Diagnosis_8 == "Z955 - Presence of coronary angioplasty implant and graft" ~ NA,
      Diagnosis_8 == "Z961 - Presence of intraocular lens" ~ NA,
      Diagnosis_8 == "Z988 - Other specified postsurgical states" ~ NA,
      TRUE ~ Diagnosis_8)) %>%
  select(-Diagnosis_8)

stroke1 <- stroke1 %>%
  mutate(
    Diagnosis_09 = case_when(
      Diagnosis_9 == "Z014 - Gynaecological examination (general)(routine)" ~ NA,
      Diagnosis_9 == "Z016 - Radiological examination, not elsewhere classified" ~ NA,
      Diagnosis_9 == "Z030 - Observation for suspected tuberculosis" ~ NA,
      Diagnosis_9 == "Z031 - Observation for suspected malignant neoplasm" ~ NA,
      Diagnosis_9 == "Z038 - Observation for other suspected diseases and conditions" ~ NA,
      Diagnosis_9 == "Z039 - Observation for suspected disease or condition, unspecified" ~ NA,
      Diagnosis_9 == "Z041 - Examination and observation following transport accident" ~ NA,
      Diagnosis_9 == "Z048 - Examination and observation for other specified reasons" ~ NA,
      Diagnosis_9 == "Z080 - Follow-up examination after surgery for malignant neoplasm" ~ NA,
      Diagnosis_9 == "Z088 - Follow-up examination after other treatment for malignant neoplasm" ~ NA,
      Diagnosis_9 == "Z089 - Follow-up examination after unspecified treatment for malignant neoplasm" ~ NA,
      Diagnosis_9 == "Z090 - Follow-up examination after surgery for other conditions" ~ NA,
      Diagnosis_9 == "Z091 - Follow-up examination after radiotherapy for other conditions" ~ NA,
      Diagnosis_9 == "Z092 - Follow-up examination after chemotherapy for other conditions" ~ NA,
      Diagnosis_9 == "Z093 - Follow-up examination after psychotherapy" ~ NA,
      Diagnosis_9 == "Z094 - Follow-up examination after treatment of fracture" ~ NA,
      Diagnosis_9 == "Z097 - Follow-up examination after combined treatment for other conditions" ~ NA,
      Diagnosis_9 == "Z098 - Follow-up examination after other treatment for other conditions" ~ NA,
      Diagnosis_9 == "Z099 - Follow-up examination after unspecified treatment for other conditions" ~ NA,
      Diagnosis_9 == "Z209 - Contact with and exposure to unspecified communicable disease" ~ NA,
      Diagnosis_9 == "Z291 - Prophylactic immunotherapy" ~ NA,
      Diagnosis_9 == "Z301 - Insertion of (intrauterine) contraceptive device" ~ NA,
      Diagnosis_9 == "Z340 - Supervision of normal first pregnancy" ~ NA,
      Diagnosis_9 == "Z348 - Supervision of other normal pregnancy" ~ NA,
      Diagnosis_9 == "Z349 - Supervision of normal pregnancy, unspecified" ~ NA,
      Diagnosis_9 == "Z351 - Supervision of pregnancy with history of abortive outcome" ~ NA,
      Diagnosis_9 == "Z358 - Supervision of other high-risk pregnancies" ~ NA,
      Diagnosis_9 == "Z359 - Supervision of high-risk pregnancy, unspecified" ~ NA,
      Diagnosis_9 == "Z370 - Single live birth" ~ NA,
      Diagnosis_9 == "Z390 - Care and examination immediately after delivery" ~ NA,
      Diagnosis_9 == "Z392 - Routine postpartum follow-up" ~ NA,
      Diagnosis_9 == "Z419 - Procedure for purposes other than remedying health state, unspecified" ~ NA,
      Diagnosis_9 == "Z429 - Follow-up care involving plastic surgery, unspecified" ~ NA,
      Diagnosis_9 == "Z470 - Follow-up care involving removal of fracture plate and other internal fixation device" ~ NA,
      Diagnosis_9 == "Z478 - Other specified orthopaedic follow-up care" ~ NA,
      Diagnosis_9 == "Z479 - Orthopaedic follow-up care, unspecified" ~ NA,
      Diagnosis_9 == "Z480 - Attention to surgical dressings and sutures" ~ NA,
      Diagnosis_9 == "Z488 - Other specified surgical follow-up care" ~ NA,
      Diagnosis_9 == "Z489 - Surgical follow-up care, unspecified" ~ NA,
      Diagnosis_9 == "Z490 - Preparatory care for dialysis" ~ NA,
      Diagnosis_9 == "Z491 - Extracorporeal dialysis" ~ NA,
      Diagnosis_9 == "Z492 - Other dialysis" ~ NA,
      Diagnosis_9 == "Z500 - Cardiac rehabilitation" ~ NA,
      Diagnosis_9 == "Z501 - Other physical therapy" ~ NA,
      Diagnosis_9 == "Z502 - Alcohol rehabilitation" ~ NA,
      Diagnosis_9 == "Z503 - Drug rehabilitation" ~ NA,
      Diagnosis_9 == "Z504 - Psychotherapy, not elsewhere classified" ~ NA,
      Diagnosis_9 == "Z505 - Speech therapy" ~ NA,
      Diagnosis_9 == "Z506 - Orthoptic training" ~ NA,
      Diagnosis_9 == "Z507 - Occupational therapy and vocational rehabilitation, not elsewhere classified" ~ NA,
      Diagnosis_9 == "Z508 - Care involving use of other rehabilitation procedures" ~ NA,
      Diagnosis_9 == "Z509 - Care involving use of rehabilitation procedure, unspecified" ~ NA,
      Diagnosis_9 == "Z510 - Radiotherapy session" ~ NA,
      Diagnosis_9 == "Z511 - Chemotherapy session for neoplasm" ~ NA,
      Diagnosis_9 == "Z512 - Other chemotherapy" ~ NA,
      Diagnosis_9 == "Z513 - Blood transfusion (without reported diagnosis)" ~ NA,
      Diagnosis_9 == "Z514 - Preparatory care for subsequent treatment, not elsewhere classified" ~ NA,
      Diagnosis_9 == "Z515 - Palliative care" ~ NA,
      Diagnosis_9 == "Z516 - Desensitization to allergens" ~ NA,
      Diagnosis_9 == "Z518 - Other specified medical care" ~ NA,
      Diagnosis_9 == "Z519 - Medical care, unspecified" ~ NA,
      Diagnosis_9 == "Z549 - Convalescence following unspecified treatment" ~ NA,
      Diagnosis_9 == "Z590 - Homelessness" ~ NA,
      Diagnosis_9 == "Z596 - Low income" ~ NA,
      Diagnosis_9 == "Z719 - Counselling, unspecified" ~ NA,
      Diagnosis_9 == "Z760 - Issue of repeat prescription" ~ NA,
      Diagnosis_9 == "Z801 - Family history of malignant neoplasm of trachea, bronchus and lung" ~ NA,
      Diagnosis_9 == "Z898 - Acquired absence of upper and lower limbs [any level]" ~ NA,
      Diagnosis_9 == "Z908 - Acquired absence of other organs" ~ NA,
      Diagnosis_9 == "Z941 - Heart transplant status" ~ NA,
      Diagnosis_9 == "Z955 - Presence of coronary angioplasty implant and graft" ~ NA,
      Diagnosis_9 == "Z961 - Presence of intraocular lens" ~ NA,
      Diagnosis_9 == "Z988 - Other specified postsurgical states" ~ NA,
      TRUE ~ Diagnosis_9)) %>%
  select(-Diagnosis_9)
...
stroke1 <- stroke1 %>%
  mutate(Diagnosa = paste0(as.character(Diagnosis_01),";",
                           ...
                           as.character(Diagnosis_08),";",
                           as.character(Diagnosis_09)))

stroke1$Diagnosa <- as.character(trimws(gsub("NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA|;NA;NA;NA;NA|;NA;NA;NA|;NA;NA|;NA|NA;","",stroke1$Diagnosa)),"both")
stroke1 <- stroke1 %>%
  select(-Diagnosis_01,-Diagnosis_02,-Diagnosis_03,-Diagnosis_04,-Diagnosis_05,
         -Diagnosis_06,-Diagnosis_07,-Diagnosis_08,-Diagnosis_09,-Diagnosis)
stroke1 <- stroke1 %>% mutate(Diagnosa = na_if(Diagnosa, ""))

stroke2 <- concat.split(stroke1, "Diagnosa", ";")
colnames(stroke2)
