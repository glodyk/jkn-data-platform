...
data <- rbind(data24,dataapr24,datamei24,dataagu24,datanov24)
rm(data24,dataapr24,datamei24,dataagu24,datanov24)
...
write.csv(data, "D://data gresik//MTF KC GRESIK//gresik_2020//hemofiliarief.csv",
          na="", row.names = FALSE)

setwd("D:/Downloads")

nokapst <- xl.read.file("KK Hemofilia KC Gresik All RS.xlsx",
                          header = TRUE,
                          row.names = NULL,
                          col.names = NULL,
                          xl.sheet = "ALL DATA",
                          ...
                          na = "",
                          excel.visible = FALSE) %>%
  select(Nosjp,NOKA,`Nama Peserta`) %>%
  rename(NamaPst = `Nama Peserta`,
         Noka = NOKA)
...
data0 <- data %>%
  mutate(
    Noka = case_when(
      Nokapst == "61701914" ~ "0000199125178",
      Nokapst == "455499014" ~ "0003108366955",
      Nokapst == "52496529" ~ "0002231312297",
      Nokapst == "29236887" ~ "0000760296756",
      Nokapst == "28858929" ~ "0000759476507",
      Nokapst == "73460320" ~ "0000103899385",
      Nokapst == "95341554" ~ "0000758449618",
      Nokapst == "329953575" ~ "0002748856173",
      Nokapst == "447122282" ~ "0002392463777",
      TRUE ~ Nokapst))

data0 <- data0 %>% 
  mutate(
    NamaPst = case_when(
      Nokapst == "61701914" ~ "ACHMAD MUNIF ELHAZMI MAHNUN",
      Nokapst == "455499014" ~ "FAREL ARDIANSYAH",
      Nokapst == "52496529" ~ "FIRMAN RADI HANNAFIANSYAH",
      Nokapst == "29236887" ~ "MUHAMMAD SYAHRUL",
      Nokapst == "28858929" ~ "SETIA HADI SUTRISNO",
      Nokapst == "73460320" ~ "ACHMAD SHOFWAN",
      Nokapst == "95341554" ~ "M. DAFIN MAULANA JAMIL",
      Nokapst == "329953575" ~ "NAUFAL MAULANA SYARIF",
      Nokapst == "447122282" ~ "SYAM SABILAL FALACH",
      TRUE ~ Nokapst)) 

data0 <- left_join(data0,nokapst, by = c("Nosjp"="Nosjp"))
data0$Noka <- with(data0,
              ifelse(is.na(data0$Noka.y), Noka.x,Noka.y))
data0$NamaPst <- with(data0,
              ifelse(is.na(data0$NamaPst.y), NamaPst.x,NamaPst.y))

data0 <- data0 %>%
  select(-NamaPst.x,-Noka.x,-NamaPst.y,-Noka.y,-Nokapst)

kosong <- data %>%
  subset(is.na(NamaPst)) %>%
  select(Nokapst) %>% unique()

%>%
  select(-Nokapst)

data_aak <- data %>% subset(Tglstjkeu >= "2023-09-01")
data_fraud <- data %>% subset(Tglpelayanan >= "2021-01-01")

data_aak <- left_join(data_aak,nokapst, by = c("Nosjp"="Nosjp")) %>%
  select(-Nokapst)
data_fraud <- left_join(data_fraud,nokapst, by = c("Nosjp"="Nosjp")) %>%
  select(-Nokapst)

data <- data %>%
  mutate(NamaPst = as.character(NamaPst),
         ALAMAT = as.character(ALAMAT),
         NOKA = as.factor(NOKA))
...
#========================================================================
write.xlsx(data, file = "data_hemofilia.xlsx")
write.xlsx(data_aak, file = "data_aak.xlsx")
write.xlsx(data_fraud, file = "data_fraud.xlsx")
#========================================================================
...
data <- data %>%
  select(Nokapst,Noka,NamaPst,Umur,Jkpst,Nmppkperujuk,Nmdati2Layan,Nmppklayan,Nosjp,
...
data <- data %>% group_by (Nokapst,Tglpelayanan) %>% 
...
#========================================================================
write.xlsx(data, file = "Hemofilia_all.xlsx")
#========================================================================
...
cek <- cek %>% group_by (Noka,Tglpelayanan) %>% 
  dplyr :: mutate (kunjungan =  sequence(n()))
#========================================================================
write.xlsx(data, file = "Phantom Billing Hemofilia(1).xlsx")
#========================================================================
