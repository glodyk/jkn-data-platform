...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  filter(str_detect(Nmdiagprimer,c("Anaemia|anaemia")))
...
df_jan19 <- read_csv("Sheet_1_Full_Data_data gresik jan 19.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Nmtkp,Nmppklayan,Tgldtgsjp,Tglplgsjp,
          ...
          Namadpjp,Nmjnspulang,biayars,Biayaverifikasi,Kelasrsmenkes) %>%
  filter(str_detect(Kelasrsmenkes,c("B|b"))) %>%
  filter(str_detect(Procedure,c("K-4-17-I")))
...
df_jul19 <- read_csv("Sheet_1_Full_Data_data gresik jul 19.csv") %>%
  ...
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Kdinacbgs,c("L-I-40")))
