...
dmcomp <- data %>%
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Kdinacbgs, "N-4-10"))

dmcomp <- dmcomp %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
dmcomp <- dmcomp %>% mutate(Diagsekunder = na_if(Diagsekunder, ""))
dmcomp <- dmcomp %>% mutate(Procedure = na_if(Procedure, "-"))
dmcomp <- dmcomp %>% mutate(Procedure = na_if(Procedure, ""))

kronis <- kronis[order(kronis$Nokapst,kronis$Tglplgsjp),]
kronis <- kronis %>% group_by(Nokapst) %>%
  mutate(tgl_before = lag(Tglplgsjp, n=1))
kronis$jeda <- as.integer(kronis$Tgldtgsjp) - as.integer(kronis$tgl_before)
kronis$jeda <- as.numeric(kronis$jeda)

kronis <- kronis[order(kronis$Nokapst,kronis$Tgldtgsjp),]
kronis <- kronis %>%
  group_by(Nokapst) %>% 
  mutate(pertama = first(Tgldtgsjp))
kronis$seminggu <- as.Date(kronis$pertama) + 7

sumperminggu <- kronis %>%
  as_tibble() %>% 
  mutate(Week = week(ymd(Tglplgsjp))) %>% 
  mutate(Month = month(ymd(Tglplgsjp))) %>% 
  mutate(Year = year(ymd(Tglplgsjp))) %>%
  group_by(Nokapst, Year, Week) %>% summarise(Count = n())

kronis <- kronis[order(kronis$Nokapst,kronis$Tglplgsjp),]
kronis <- kronis %>%
  group_by(Nokapst) %>%
  mutate(kunjung_ke = sequence(n()))
kronis$kunjung_ke <- abs(kronis$kunjung_ke )
#========================================================================
write.csv(dmcomp, "D:/data gresik/MTF KC GRESIK/gresik_2020/N_4_10.csv",
          na="", row.names = FALSE)

irisan apabila jeda 1,0,negatif
#========================================================================
