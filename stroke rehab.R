...
data19 <- read_csv("Sheet 1_Full Data gresik 2019.csv") %>%
  ...
  filter(str_detect(Diagnosa,"I63|I631|I632|I633|I634|I635|I636|
                    I637|I638|I639|I64")) %>%
  select(-Diagnosa)
...
rm(data19,data20,data21,data22,data23,data24,dataapr24,datamei24)
...
data <- left_join(data,refpoli, by = c("Politujsjp"="KDPOLI")) %>%
  select(-Politujsjp)
data <- data %>% rename(Politujsjp = NMPOLI)
rm(refpoli)
#======================================================================================
nokapst <- data %>% select(Nokapst)
nokapst <- nokapst %>% distinct(Nokapst, .keep_all = TRUE)
#======================================================================================
setwd("D:/data gresik/MTF KC GRESIK/gresik_2020")
...
df <- rbind(data18,data19,data20,data21,data22,data23,data24,dataapr24,datamei24)
rm(data18,data19,data20,data21,data22,data23,data24,dataapr24,datamei24)

df$Tgldtgsjp <- as.Date(df$Tgldtgsjp, format = "%m/%d/%Y")
df$Tglplgsjp <- as.Date(df$Tglplgsjp, format = "%m/%d/%Y")
df$Tglpelayanan <- as.Date(df$Tglpelayanan, format = "%m/%d/%Y")
...
df <- df[order(df$Nokapst,df$Tglplgsjp),]

df <- left_join(nokapst,df, by = c("Nokapst"="Nokapst"))
df <- df[!is.na(df$Nosjp), ]

df$Keterangan <- "Serangan Stroke"

datanew <- rbind(data,df)
datanew <- datanew[order(datanew$Nokapst,datanew$Tglplgsjp),]

datanew <- datanew %>%
  group_by(Nokapst) %>%
  mutate(Keterangan1 = lag(Keterangan, n = 1))
datanew <- datanew %>%
  group_by(Nokapst) %>%
  mutate(Keterangan2 = lead(Keterangan, n = 1))
datanew$Keterangan3 <- paste0(as.character(datanew$Keterangan1),";",
                              as.character(datanew$Keterangan),";",
                              as.character(datanew$Keterangan2))

datanew <- subset(datanew,
                  !(Keterangan3 %in% c("NA;Fisiotx;NA",
                                       "Fisiotx;Serangan Stroke;NA",
                                       "NA;Serangan Stroke;Serangan Stroke",
                                       "NA;Fisiotx;Serangan Stroke",
                                       "Fisiotx;Serangan Stroke;NA")))

datanew <- datanew[order(datanew$Nokapst,datanew$Tglplgsjp),]

datanew$Keterangan4 <- with(datanew,
                            ifelse(Keterangan3 == "NA;Serangan Stroke;Fisiotx","MULAI",
                                   ifelse(Keterangan3 == "NA;Fisiotx;Fisiotx","MULAI",
                                          ifelse(Keterangan3 == "NA;Fisiotx;Serangan Stroke","MULAI",
                                                 ifelse(Keterangan3 == "Serangan Stroke;Serangan Stroke;Fisiotx","MULAI",
                                                        ifelse(Keterangan3 == "Fisiotx;Serangan Stroke;Fisiotx","BARU",
                                                               ifelse(Keterangan3 == "Fisiotx;Serangan Stroke;Serangan Stroke","BARU",
                                                                      ifelse(Keterangan3 == "Fisiotx;Serangan Stroke;NA","SELESAI",
                                                                             ifelse(Keterangan3 == "Serangan Stroke;Serangan Stroke;NA","SELESAI",
                                                                                    ifelse(Keterangan3 == "Fisiotx;Fisiotx;NA","SELESAI",
                                                                                           ifelse(Keterangan3 == "Fisiotx;Fisiotx;Serangan Stroke","SELESAI",
                                                                                                NA)))))))))))
...
datanew <- datanew %>%
  group_by(Nokapst) %>%
  mutate(Keterangan5 = lag(Keterangan4, n = 1))

datanew$Keterangan6 <- paste0(as.character(datanew$Keterangan5),";",
                              as.character(datanew$Keterangan4))
datafinal <- datanew %>%
  filter(!str_detect(Keterangan6,c("BARU"))) %>%
  select(Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nosjp,Nmtkp,
         Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,
         Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,
         Namadpjp,Nmjnspulang,biayars,Biayaverifikasi,Keterangan)

datafinal <- datafinal[order(datafinal$Nokapst,datafinal$Tglplgsjp),]
datafinal <- datafinal %>%
  group_by(Nokapst) %>% 
  mutate(interval = Tglplgsjp - first(Tglplgsjp))
datafinal$Flag <- with(datafinal,
                       ifelse(interval == 0, "Dianggap mulai Stroke",
                              ifelse(interval <= 180, "Sesuai Perdosri",
                                     "Tidak Sesuai")))
#============================================================================
datanew2 <- datanew %>%
  filter(str_detect(Keterangan6,c("BARU"))) %>%
...
datanew2 <- datanew2[order(datanew2$Nokapst,datanew2$Tglplgsjp),]
...
datanew2 <- datanew2 %>%
  group_by(Nokapst) %>%
  mutate(Keterangan2 = lead(Keterangan, n = 1))
datanew2$Keterangan3 <- paste0(as.character(datanew2$Keterangan1),";",
                              as.character(datanew2$Keterangan),";",
                              as.character(datanew2$Keterangan2))

datanew2 <- subset(datanew2,
                  !(Keterangan3 %in% c("NA;Fisiotx;NA",
                                       "Fisiotx;Serangan Stroke;NA",
                                       "NA;Serangan Stroke;Serangan Stroke",
                                       "NA;Fisiotx;Serangan Stroke",
...
datanew2 <- datanew2[order(datanew2$Nokapst,datanew2$Tglplgsjp),]
datanew2$Keterangan4 <- with(datanew2,
                            ifelse(Keterangan3 == "NA;Serangan Stroke;Fisiotx","MULAI",
                                   ifelse(Keterangan3 == "NA;Fisiotx;Fisiotx","MULAI",
                                          ifelse(Keterangan3 == "NA;Fisiotx;Serangan Stroke","MULAI",
                                                 ifelse(Keterangan3 == "Serangan Stroke;Serangan Stroke;Fisiotx","MULAI",
...
datanew2$Keterangan6 <- paste0(as.character(datanew2$Keterangan5),";",
                              as.character(datanew2$Keterangan4))
datafinal2 <- datanew2 %>%
  filter(!str_detect(Keterangan6,c("BARU"))) %>%
  select(Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nosjp,Nmtkp,
         Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,
...
datafinal2 <- datafinal2[order(datafinal2$Nokapst,datafinal2$Tglplgsjp),]
datafinal2 <- datafinal2 %>%
  group_by(Nokapst) %>% 
  mutate(interval = Tglplgsjp - first(Tglplgsjp))
datafinal2$Flag <- with(datafinal2,
                       ifelse(interval == 0, "Dianggap mulai Stroke",
                              ifelse(interval <= 180, "Sesuai Perdosri",
                                     "Tidak Sesuai")))
#============================================================================
...
datanew3 <- datanew3[order(datanew3$Nokapst,datanew3$Tglplgsjp),]
...
datanew3 <- datanew3 %>%
  group_by(Nokapst) %>%
  mutate(Keterangan2 = lead(Keterangan, n = 1))
datanew3$Keterangan3 <- paste0(as.character(datanew3$Keterangan1),";",
                               as.character(datanew3$Keterangan),";",
                               as.character(datanew3$Keterangan2))

datanew3 <- subset(datanew3,
                   !(Keterangan3 %in% c("NA;Fisiotx;NA",
                                        "Fisiotx;Serangan Stroke;NA",
                                        "NA;Serangan Stroke;Serangan Stroke",
                                        "NA;Fisiotx;Serangan Stroke",
...
datanew3 <- datanew3[order(datanew3$Nokapst,datanew3$Tglplgsjp),]
datanew3$Keterangan4 <- with(datanew3,
                             ifelse(Keterangan3 == "NA;Serangan Stroke;Fisiotx","MULAI",
                                    ifelse(Keterangan3 == "NA;Fisiotx;Fisiotx","MULAI",
                                           ifelse(Keterangan3 == "NA;Fisiotx;Serangan Stroke","MULAI",
                                                  ifelse(Keterangan3 == "Serangan Stroke;Serangan Stroke;Fisiotx","MULAI",
...
datanew3$Keterangan6 <- paste0(as.character(datanew3$Keterangan5),";",
                               as.character(datanew3$Keterangan4))
datafinal3 <- datanew3 %>%
  filter(!str_detect(Keterangan6,c("BARU"))) %>%
  select(Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nosjp,Nmtkp,
         Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,
...
datafinal3 <- datafinal3[order(datafinal3$Nokapst,datafinal3$Tglplgsjp),]
datafinal3 <- datafinal3 %>%
  group_by(Nokapst) %>% 
  mutate(interval = Tglplgsjp - first(Tglplgsjp))
datafinal3$Flag <- with(datafinal3,
                        ifelse(interval == 0, "Dianggap mulai Stroke",
                               ifelse(interval <= 180, "Sesuai Perdosri",
                                      "Tidak Sesuai")))
#============================================================================
dfix <- rbind(datafinal,datafinal2,datafinal3)
dfix <- dfix[order(dfix$Nokapst,dfix$Tglplgsjp),]
dfix <- dfix %>%
  select(Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nosjp,Nmtkp,
         Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,interval,Kdinacbgs,
         Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,
         Namadpjp,Nmjnspulang,biayars,Biayaverifikasi,Keterangan,Flag)
rm(data,df,datafinal,datafinal2,datafinal3,datanew,datanew2,datanew3,nokapst)
#========================================================================
write.xlsx(dfix, file = "stroke rehab_RS Semen.xlsx")
#========================================================================
...
df_data$Keterangan1 <- with(df_data,
                            ifelse(Rehab_ke == "Stroke",1,0))

df_data$Keterangan <- with(df_data,
                           ifelse(jeda <= 180, "Sesuai Perdosri",
                                  "Tidak Sesuai"))

datanew <- datanew %>%
  mutate(Keterangan5 = case_when(
    str_detect(Keterangan4, "-NA") ~ NA,
    TRUE ~ Keterangan5))  

datanew$Keterangan2 <- with(datanew,
                            ifelse(is.na(datanew$Keterangan2),
                                   Keterangan,Keterangan2))



datanew$Keterangan1 <- with(datanew,
                            ifelse(is.na(datanew$Keterangan1),
...
datanew$Keterangan5 <- paste0(as.character(datanew$Nokapst),"-",
                              as.character(datanew$Keterangan4))
...
datafinal <- data %>%
  filter(str_detect(keterangan3,c("Persalinan;NA|Kontrol tanpa rawat luka;Persalinan")))



datanew$flag <- with(datanew,
                     ifelse(kunj_ke == 1, sequence(n()),
                            kunj_ke))


data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>% group_by(Nokapst) %>%
  mutate(kunj_ke = sequence(n()))

datafinal <- datafinal %>%
  group_by(Nokapst) %>%
  mutate(keterangan2 = lag(keterangan, n = 1))

datafinal$keterangan3 <- paste0(as.character(datafinal$keterangan),";",
                                as.character(datafinal$keterangan2))

datafinal <- datafinal %>%
  filter(str_detect(keterangan3,c("Persalinan;NA|USG;Persalinan")))


df <- df[order(df$Nokapst,df$Tglplgsjp),]
df <- df %>% group_by(Nokapst) %>%
  mutate(kunj_ke = sequence(n()))
df_data <- rbind(data,df)
df_data <- df_data[order(df_data$Nokapst,df_data$Tglplgsjp),]
rm(data,df)

df_data <- df_data %>%
  group_by(Nokapst, kunj_ke) %>%
  mutate(constant = 1, 
         index = cumsum(constant),
         interval = Tglplgsjp - first(Tglplgsjp)) %>% 
  filter(interval <= 3) %>% 
  select(-constant)

df_data <- df_data %>% group_by(Nokapst) %>%
  mutate(flag = sequence(n()))

df_data$flag <- with(df_data,
                           ifelse(kunj_ke == 1, sequence(n()),
                                  kunj_ke))


df$Rehab_ke <- "Stroke"
data$Rehab_ke <- as.factor(data$Rehab_ke)
...
df_data <- df_data %>% 
  group_by(Nokapst) %>% 
  mutate(jeda = Tglplgsjp - first(Tglplgsjp))
df_data$jeda <- as.numeric(df_data$jeda)

df_data$Keterangan <- with(df_data,
                        ifelse(jeda <= 180, "Sesuai Perdosri",
                               "Tidak Sesuai"))
df_data <- df_data %>% select(-jeda)

df_data <- df_data %>% select(Nokapst,Umur,Jkpst,Nosjp,Nmdati2Layan,
                              Nmppklayan,Nmtkp,Politujsjp,Tglpelayanan,
                              Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
                              Kddiagprimer,Nmdiagprimer,Diagsekunder,
                              Procedure,Nmjnspulang,biayars,
                              Biayaverifikasi,Rehab_ke,interval,Keterangan)
df_data <- df_data[order(df_data$Nokapst,df_data$Tglplgsjp),]

write.xlsx(df_data, file = "Stroke Fisio.xlsx")


df_data$Keterangan <- as.integer(ifelse(data$netbiaya <= 0, 1, 0))
