...
dmcomp <- data %>%
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Diagnosa, "E115"))

kronis <- kronis %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
kronis <- kronis %>% mutate(Diagsekunder = na_if(Diagsekunder, ""))
kronis <- kronis %>% mutate(Procedure = na_if(Procedure, "-"))
kronis <- kronis %>% mutate(Procedure = na_if(Procedure, ""))
...
#========================================================================
write.csv(dmcomp, "D:/data gresik/MTF KC GRESIK/gresik_2020/E115.csv",
          na="", row.names = FALSE)
