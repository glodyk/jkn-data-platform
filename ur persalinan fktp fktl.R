...
data23a <- read_csv("Sheet 1_Full Data gresik 2023.csv") %>%
  select (Nokapst,Nosjp,Nmtkp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
          Nmdati2Layan,Nmppklayan,Kdinacbgs,Diagsekunder,Procedure,
  ...
  mutate(Keterangan = "Persalinan FKTL") %>%
  select(-Nmtkp,-Kdinacbgs,-Diagsekunder,-Procedure)
...
data25a <- read_csv("Sheet 1_Full Data gresik apr 2024.csv") %>%
  select (Nokapst,Nosjp,Nmtkp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
          Nmdati2Layan,Nmppklayan,Kdinacbgs,Diagsekunder,Procedure,
          Nmjnspulang,Biayaverifikasi) %>%
  subset(Nmtkp == "RITL") %>%
...
data25b <- read_csv("Sheet 1_Full Data gresik apr 2024.csv") %>%
  select (Nokapst,Nosjp,Nmtkp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
          Nmdati2Layan,Nmppklayan,Kdinacbgs,Diagsekunder,Procedure,
          Nmjnspulang,Biayaverifikasi) %>%
  subset(Nmtkp == "RITL") %>%
...
data <- rbind(data23a,data23b,data24a,data24b,data25a,data25b)
rm(data23a,data23b,data24a,data24b,data25a,data25b)

data <- data %>%
  group_by(Nosjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Nmdati2Layan,
           Nmppklayan,Nmjnspulang,Biayaverifikasi,Keterangan) %>% slice(1)
data <- data[!duplicated(data$Nosjp, fromLast = FALSE),]
data$Keterangan <- with(data,
                        ifelse(is.na(data$Keterangan), "Rawat Inap FKTL",
                               Keterangan))
...
data <- data %>%
  subset(Tglpelayanan >= "2023-01-01")

nonkappersalinan <- read_csv("ur_nonkap.csv") %>%
  select(Nokapst,Nosjp,Tglpelayanan,Tglkunjungan,Tglpulang,Nmdati2Layan,
         Nmppklayan,Nmstatuspulang,Biaya,Klasifikasi) %>%
  subset(Klasifikasi == "Persalinan") %>%
  select(-Klasifikasi) %>%
  mutate (Keterangan = "Persalinan FKTP") %>%
  rename(Tgldtgsjp = Tglkunjungan,
         Tglplgsjp = Tglpulang,
         Nmjnspulang = Nmstatuspulang)

nonkapranap <- read_csv("ur_nonkap.csv") %>%
  select(Nokapst,Nosjp,Tglpelayanan,Tglkunjungan,Tglpulang,Nmdati2Layan,
         Nmppklayan,Nmstatuspulang,Biaya,Klasifikasi) %>%
  subset(Klasifikasi == "Rawat Inap") %>%
  select(-Klasifikasi) %>%
  mutate (Keterangan = "Rawat Inap FKTP") %>%
  rename(Tgldtgsjp = Tglkunjungan,
         Tglplgsjp = Tglpulang,
         Nmjnspulang = Nmstatuspulang)

nonkap <- rbind(nonkappersalinan,nonkapranap) %>%
  rename(Biayaverifikasi = Biaya)
rm(nonkappersalinan,nonkapranap)

nonkap <- nonkap %>%
  subset(Tglpelayanan >= "2023-01-01")

df <- rbind(data,nonkap)
rm(data,nonkap)

# Check for overlaps
df1 <- df %>%
  arrange(Nokapst, Tgldtgsjp, Tglplgsjp) %>% 
  group_by(Nokapst) %>%
  summarise(overlapping = any(Tgldtgsjp < lag(Tglplgsjp,
                                              default = first(Tglplgsjp)) & row_number() != 1)) %>%
  subset(overlapping == TRUE) %>%
  select(Nokapst)

#mencari pasien usg pada data persalinan (persalinan saja)
overlap <- left_join(df1,df,by = "Nokapst")
overlap <- overlap[order(overlap$Nokapst,
                         overlap$Tglplgsjp),]
overlap <- overlap %>%
  group_by(Nokapst) %>%
  mutate(selisih = Tgldtgsjp - lag(Tglplgsjp))

write.xlsx(overlap, file = "overlap.xlsx")
#==============================================================================
write.csv(df, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_total.csv",
          na="", row.names = FALSE)
#========================================================================
#========================================================================
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>% group_by(Nokapst) %>%
  mutate(persalinan_ke = sequence(n()))
data$Flag <- with(data,
...
data$interval2 <- with(data,
                       ifelse(is.na(data$interval), lead(data$interval),
                              data$interval))
data$ket2 <- with(data,
                       ifelse(data$interval2 < 280, "salah","benar"))
data$ket3 <- with(data,
                   ifelse(data$Nokapst == lead(data$Nokapst),
...
data <- data %>%
  select(Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Nmdati2Layan,Nmppklayan,
         Klsrawat,Tgldtgsjp,Tglplgsjp,interval,Kdinacbgs,
         Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,
         Nmjnspulang,persalinan_ke,biayars,Biayaverifikasi)

data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>% group_by(Nokapst) %>%
  mutate(interval = as.numeric(Tgldtgsjp-lag(Tglplgsjp)))

write.xlsx(data, file = "persalinan_dobel.xlsx")
#========================================================================
