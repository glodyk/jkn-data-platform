...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  ...
  filter(str_detect(Kdinacbgs,c("U-3-16-0"))) %>%
  filter(str_detect(Namadpjp,c("ORT|ort")))
...
data <- data %>% select(Nokapst,Nosjp,Jkpst,Umur,NmFKRTL,Tgldtgsjp,
                        Tglplgsjp,Politujsjp,Namadpjp,Kdinacbgs,
                        Kddiagprimer,Nmdiagprimer,
                        Diagsekunder,Procedure,
...
write.xlsx(data, file = "U-3-16-0.xlsx")
