...
data18 <- read_csv("Sheet 1_Full Data gresik 2018.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Kelasrsmenkes,Kepemilikanppklayan,
          Kdppklayan,Nmdati2Layan,Nmtkp,Klsrawat,Tglpelayanan,
          Tgldtgsjp,Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,biayars,Biayaverifikasi,Tarifgroup,
          kdsp,tarifsp,deskripsisp,kdsr,tarifsr,deskripsisr) %>%
  subset(kdsp != "-") %>%
  subset(kdsp != "None") %>%
  subset(kdsp != "NONE") %>%
  subset(kdsr != "-") %>%
...
data24apr <- read_csv("Sheet 1_Full Data gresik apr 2024.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Kelasrsmenkes,Kepemilikanppklayan,
          Kdppklayan,Nmdati2Layan,Nmtkp,Klsrawat,Tglpelayanan,
          Tgldtgsjp,Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,biayars,Biayaverifikasi,Tarifgroup,
...
data24mei <- read_csv("Sheet 1_Full Data gresik mei 2024.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Kelasrsmenkes,Kepemilikanppklayan,
          Kdppklayan,Nmdati2Layan,Nmtkp,Klsrawat,Tglpelayanan,
          Tgldtgsjp,Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,biayars,Biayaverifikasi,Tarifgroup,
...
data <- rbind(data18,data19,data20,data21,data22,data23,data24,data24apr,
              data24mei)
rm(data18,data19,data20,data21,data22,data23,data24,data24apr,data24mei)
...
data <- data %>% subset(Tglplgsjp >= "2018-01-01")

data$Diagsekunder [is.na(data$Diagsekunder)] <- "-"

data <- data %>%
  mutate(Diagnosa = paste0(as.character(data$Kddiagprimer)," - ",
                           as.character(data$Nmdiagprimer),"; ",
                           as.character(data$Diagsekunder)))
data$Diagnosa <- as.character(trimws(gsub("; -","",data$Diagnosa)), "both")
...
datkorx <- left_join(data,datkor, by = c("Nosjp","Nmtkp","Kdinacbgs")) %>%
  select(-Tglsep,-Nminacbgs,-BYTAGSEP)

datkorx$Ket_Diag <- with(datkorx,
                         ifelse(str_detect(Diagnosa, c("Coxarthrosis|coxarthrosis|pelvic|neck of femur|trochanter|hip")),"Diagnosa Hip",
                                ifelse(str_detect(Diagnosa, c("T818|T844|T847")),"adverse preventable effect",
                                       "Diagnosa Knee")))

datkorx$Ket_Proc <- with(datkorx,
                           ifelse(str_detect(Procedure, c("8151|8152|8153")),"Prosedur Hip",
                                  ifelse(str_detect(Procedure, c("8154")),"Prosedur Knee",
                                         "Salah")))
datkorx$Diagsekunder <- as.character(trimws(gsub("-","",datkorx$Diagsekunder)), "both")

datkorx <- datkorx %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Kelasrsmenkes,Kepemilikanppklayan,
          Nmppklayan,Nmdati2Layan,Nmtkp,Klsrawat,Tglpelayanan,
          Tgldtgsjp,Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,biayars,Biayaverifikasi,Tarifgroup,
          kdsp,tarifsp,deskripsisp,kdsr,tarifsr,deskripsisr,Ket_Diag,
          Ket_Proc,KDCBGSVER,Efisiensi,BYVERSEP)

#========================================================================
write.xlsx(datkorx, file = "Topup Hip_Knee.xlsx")
#========================================================================

data <- data %>%
  mutate(Diagsekunder = na_if(Diagsekunder, "-"))
data <- data %>%
  mutate(Procedure = na_if(Procedure, "-"))



write.csv(data, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_N179_all.csv",
          na="", row.names = FALSE)
#==================================================================
