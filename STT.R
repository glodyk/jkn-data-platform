...
data20 <- read_csv("Sheet 1_Full Data gresik 2020.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Nmdati2Layan,Nmppklayan,
          ...
          Biayaverifikasi) %>% 
  subset(Nmtkp == "RITL") %>% 
  subset(Nmdati2Layan == "KAB. LAMONGAN") %>% 
  subset(Kdinacbgs == "L-1-40-I") %>% 
  subset(is.na(Diagsekunder)) %>%
  filter(str_detect(Kddiagprimer,c("D219|D239|D177|D179|D367|L024|L031|L039|M674|L910|L720"))) %>%
  filter(str_detect(Procedure,c("863|8609")))
