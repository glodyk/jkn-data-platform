...
data19 <- read_csv("Sheet 1_Full Data gresik 2019.csv") %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppkperujuk,Kdppklayan,Nosjp,
          Nmtkp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>%
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Kddiagprimer,c("I633|I634|I639"))) %>%
  filter(str_detect(Diagsekunder,c("Hemiplegi|hemiplegi")))
...
data <- data %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nmppkperujuk,Nosjp,
          Nmtkp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi)
data <- data[order(data$Nokapst,data$Tglplgsjp),]

write.xlsx(data, file = "hemiplegia.xlsx")
