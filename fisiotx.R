...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  filter(str_detect(Procedure,c("9339")))

df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  filter(str_detect(Procedure,c("1341|1371|1311|133")))

df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmppklayan,Tgldtgsjp,
          Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,Namadpjp,biayars,
          Biayaverifikasi,Politujsjp) %>%
  filter(str_detect(Politujsjp,c("IRM")))
...
data$Nokapst <- as.character(data$Nokapst)
data$keterangan <- "Fisioterapi"

data$Tgldtgsjp <- as.Date(data$Tgldtgsjp, format = "%m/%d/%Y")
data$Tglplgsjp <- as.Date(data$Tglplgsjp, format = "%m/%d/%Y")

data$keterangan0 <- paste0("fisiotx ", match(data$Nokapst,
                                             unique(data$Nokapst)))

data <- data %>%
  group_by(Nokapst) %>%
  mutate(keterangan0 = rowid(Nokapst))
data$keterangan0 <- as.numeric(data$keterangan0)
glimpse(data)

data <- data %>%
  group_by(Nokapst,Tglplgsjp) %>%
  mutate(keterangan0 = sequence(n()))

#mencari pasien fisioterapi > 40
data40 <-  data %>%
  filter(keterangan0 >= 40)
data40 <- data40[order(data40$Nokapst,
                       data40$Tglplgsjp),]


datanoka <- data40 %>% select(Nokapst)
datanoka <- datanoka[!(duplicated(datanoka) |
                         duplicated(datanoka, fromLast = TRUE)), ]

#mencari pasien kontrol post fako
fisoterapi40 <- left_join(datanoka,data, by = "Nokapst")

fisoterapi40 <- fisoterapi40 %>%
  group_by(Nokapst) %>%
  mutate(keterangan0 = sequence(n()))
