setwd("D:/data gresik/SEP approval 23")
...
df_approval1 <- read_csv("factsepterbitbiometrik_IMP_ 2122.csv") %>%
  select(Nokapst,Nosep,`Bulan Layan`,`First Date`,Tglsep,Tglplgsep,
         `Fp Approval`,`First Date Fp Approval`,`Last Date Fp Approval`,
         `Fp Sep`,`First Date Fp Sep`,`Last Date  Fp Sep`,Nmdati2Layan,
         Kdppklayan,Nmppklayan,ppkrjkawalsep,nmppkrjkawalsep,Nmtkp,
         Politujsep,`Until Now`,Fuser,jnskecelakaan,kll_kk_pak) %>%
  subset(`Fp Approval` == 1)

df_approval2 <- read_csv("factsepterbitbiometrik_IMP_ 2324.csv") %>%
  select(Nokapst,Nosep,`Bulan Layan`,`First Date`,Tglsep,Tglplgsep,
         `Fp Approval`,`First Date Fp Approval`,`Last Date Fp Approval`,
         `Fp Sep`,`First Date Fp Sep`,`Last Date  Fp Sep`,Nmdati2Layan,
         Kdppklayan,Nmppklayan,ppkrjkawalsep,nmppkrjkawalsep,Nmtkp,
...
df_approval <- rbind(df_approval1,df_approval2)
rm(df_approval1,df_approval2)

df_approval <- read_csv("factsepterbitbiometrik_IMP_ 2024.csv") %>%
  select(Nokapst,Nosep,`Bulan Layan`,`First Date`,Tglsep,Tglplgsep,
         `Fp Approval`,`First Date Fp Approval`,`Last Date Fp Approval`,
         `Fp Sep`,`First Date Fp Sep`,`Last Date  Fp Sep`,Nmdati2Layan,
         Kdppklayan,Nmppklayan,ppkrjkawalsep,nmppkrjkawalsep,Nmtkp,
...
df_approval$`Bulan Layan` <- as.Date(df_approval$`Bulan Layan`,format = "%m/%d/%Y")
df_approval$`First Date` <- as.Date(df_approval$`First Date`,format = "%m/%d/%Y")
df_approval$Tglsep <- as.Date(df_approval$Tglsep,format = "%m/%d/%Y")
df_approval$Tglplgsep <- as.Date(df_approval$Tglplgsep,format = "%m/%d/%Y")
df_approval$`First Date Fp Approval` <- as.Date(df_approval$`First Date Fp Approval`,format = "%m/%d/%Y")
df_approval$`Last Date Fp Approval` <- as.Date(df_approval$`Last Date Fp Approval`,format = "%m/%d/%Y")
df_approval$`First Date Fp Sep` <- as.Date(df_approval$`First Date Fp Sep`,format = "%m/%d/%Y")
df_approval$`Last Date  Fp Sep` <- as.Date(df_approval$`Last Date  Fp Sep`,format = "%m/%d/%Y")

df_approval <- df_approval %>%
  select(Nokapst,Nosep,Nmtkp,Politujsep,`Bulan Layan`,Tglsep,Tglplgsep,
         Nmppklayan,nmppkrjkawalsep)
df_approval <- df_approval[order(df_approval$Nokapst,df_approval$Tglplgsep),]
df_approval1 <- df_approval %>% subset(Tglsep >= "2024-04-01")

write.xlsx(df_approval1, file = "sep_approval.xlsx")
write.csv(df_approval,
          "D://data gresik//SEP approval 23//df_approval.csv",
          na="", row.names = FALSE)
