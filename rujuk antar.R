...
data19 <- read_csv("Sheet 1_Full Data gresik 2019.csv") %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Jenisppkperujuk,Typeppkperujuk,Nmppkperujuk,
          Kdppkperujuk,Kdppklayan,Kelasrsmenkes,Nosjp,Nmtkp,Politujsjp,Tglpelayanan,
          Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,
...
datajul25 <- read_csv("Sheet 1_Full Data gresik jul 2025.csv") %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Jenisppkperujuk,Typeppkperujuk,Nmppkperujuk,
          Kdppkperujuk,Kdppklayan,Kelasrsmenkes,Nosjp,Nmtkp,Politujsjp,Tglpelayanan,
          Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi)
...
data <- rbind(data19,data20,data21,data22,data23,data24,dataapr24,datamei24,
              dataagu24,datanov24,datajan25,datajul25)
rm(data19,data20,data21,data22,data23,data24,dataapr24,datamei24,dataagu24,datanov24,
   datajan25,datajul25)
...
data <- data %>% 
  mutate(
    Nmppkperujuk = case_when(
      ...
      Kdppkperujuk == "0205R013" ~ "RS PKG Grha Husada",
      Kdppkperujuk == "0205R014" ~ "RS Petrokimia Driyorejo",
      Kdppkperujuk == "0205R019" ~ "RS Wali Songo I",
      Kdppkperujuk == "0205R021" ~ "RS Fathma Medika",
      Kdppkperujuk == "0205R022" ~ "RS Wates Husada",
      Kdppkperujuk == "0205R023" ~ "RS Surya Medika",
      Kdppkperujuk == "0205R024" ~ "RS PKU Muhammadiyah Sekapuk",
      Kdppkperujuk == "0205R025" ~ "RSI Mabarrot MWC NU Bungah",
      Kdppkperujuk == "0205R026" ~ "RS Rachmi Dewi",
      Kdppkperujuk == "0205R027" ~ "RSI Nyai Ageng Pinatih",
      Kdppkperujuk == "0205R028" ~ "RSI Cahaya Giri",
      Kdppkperujuk == "0205R029" ~ "RSUD Umar Mas'ud Bawean",
      Kdppkperujuk == "0205S100" ~ "Klinik Mata Utama",
      Kdppkperujuk == "1302R001" ~ "RSUD Ibnu Sina",
      Kdppkperujuk == "1302R002" ~ "RS Petrokimia Gresik",
      Kdppkperujuk == "1306R001" ~ "RSUD Dr Soegiri",
      Kdppkperujuk == "0205R031" ~ "RS Eka Husada",
      Kdppkperujuk == "0205R030" ~ "RS Randegansari Husada",
      Kdppkperujuk == "0204R020" ~ "RS Permata Bunda",
      Kdppkperujuk == "0204R021" ~ "RSUD Karangkembang",
      Kdppkperujuk == "0204R022" ~ "RS Nahdlatul Ulama Babat",
      Kdppkperujuk == "0204R023" ~ "RS Permata Hati",
      Kdppkperujuk == "0204R024" ~ "RS Muhammadiyah Kalikapas",
      Kdppkperujuk == "0205R032" ~ "RSIA Khodijah",
      Kdppkperujuk == "0205S101" ~ "Klinik Utama Gresik",
      Kdppkperujuk == "0205R033" ~ "RSUD Gresik Sehati",
      TRUE ~ NA)) %>%
  subset(!is.na(Nmppkperujuk))
...
rujuk0 <- rujuk %>%
  select(Nokapst,Kdppklayan) %>%
  unique() %>%
  rename(Kdppkperujuk = Kdppklayan)

tidakrujuk <- data %>%
  subset(Nmtkp == "RITL") %>%
  subset(Nmjnspulang != "Rujuk")

rujukan0 <- left_join(rujuk0,tidakrujuk,
                   by = c("Nokapst"="Nokapst", "Kdppkperujuk" = "Kdppkperujuk")) %>%
  subset(!is.na(Nosjp))

rujukan1 <- rbind(rujukan0,rujuk) %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Jenisppkperujuk,Typeppkperujuk,Nmppkperujuk,
          Nmppklayan,Kelasrsmenkes,Nosjp,Nmtkp,Politujsjp,Tglpelayanan,Tgldtgsjp,
          Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi)
rm(rujukan0,rujuk0,rujuk,tidakrujuk)

rujukan1$Tgldtgsjp <- as.Date(rujukan1$Tgldtgsjp, format = "%m/%d/%Y")
rujukan1$Tglplgsjp <- as.Date(rujukan1$Tglplgsjp, format = "%m/%d/%Y")
rujukan1$Tglpelayanan <- as.Date(rujukan1$Tglpelayanan, format = "%m/%d/%Y")
rujukan1$Tglstjkeu <- as.Date(rujukan1$Tglstjkeu, format = "%m/%d/%Y")

rujukan1 <- rujukan1 %>% subset(Tglstjkeu >= "2019-01-01")

rujukan1 <- rujukan1[order(rujukan1$Nokapst,rujukan1$Tglplgsjp),]
rujukan1$flag <- with(rujukan1,
                      ifelse(lag(Nokapst) == Nokapst | lead(Nokapst) == Nokapst,
                         "bener","salah")) 
rujukan1 <-  rujukan1 %>%
  subset(flag == "bener")
rujukan1 <- rujukan1[order(rujukan1$Nokapst,rujukan1$Tglplgsjp),]
rujukan1 <- rujukan1 %>% group_by(Nokapst) %>%
  mutate(tgl_before = lag(Tglplgsjp, n=1))

rujukan1$jeda <- as.integer(rujukan1$Tgldtgsjp) - as.integer(rujukan1$tgl_before)
rujukan1$jeda <- as.numeric(rujukan1$jeda) %>%
  replace(is.na(.),0)

rujukan1 <- rujukan1[order(rujukan1$Nokapst,rujukan1$Tglplgsjp),]
rujukan1$flag2 <- with(rujukan1,
                       ifelse(jeda > 1,"salah","benar"))
rujukan1$flag3 <- with(rujukan1,
                       ifelse(lead(flag2) == "salah","salah",flag2))
rujukan1 <- rujukan1[order(rujukan1$Nokapst,rujukan1$Tglplgsjp),]
rujukan1 <- rujukan1 %>% subset(flag3 == "benar")

rujukan1 <- rujukan1 %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppkperujuk,Nmppklayan,Kelasrsmenkes,Nosjp,Nmtkp,
          Tglpelayanan,Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,
          Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,Nmjnspulang,
...
rujukan1 <- rujukan1 %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppkperujuk,Nmppklayan,Kelasrsmenkes,
          Nosjp,Nmtkp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,
          Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,Nmjnspulang,
          biayars,Biayaverifikasi)
...
rujukan1$flag <- with(rujukan1,
                      ifelse(lag(Nokapst) == Nokapst | lead(Nokapst) == Nokapst,
                         "bener","salah"))
rujukan1 <- rujukan1 %>% subset(flag == "bener") %>% select(-flag)

write.xlsx(rujukan1, file = "Bahan Audit Medis Rujukan Antar RS RITL.xlsx")
...
rujukan1 <- rujukan1 %>% subset(jeda < 7)

rujukan1$flag <- with(rujukan1,
                      ifelse(lag(Nokapst) == Nokapst & lead(Nokapst) == Nokapst,Nosjp,NA))

%>%
  select(Nokapst,Kdppklayan,Nosjp) %>%
  rename(Kdppkperujuk = Kdppklayan)
...
rujukan <- left_join(rujukan,data,
                     by = c("Nokapst"="Nokapst", "Kdppklayan" = "Kdppklayan")) %>%
  select(Nokapst,Kdppklayan,Nosjp) %>%
  rename(Kdppkperujuk = Kdppklayan)

rujukan <- left_join(rujuk,data,
                  by = c("Nokapst"="Nokapst", "Kdppkperujuk" = "Kdppkperujuk")) %>%
  select(Nokapst,Kdppklayan,Nosjp) %>%
  subset(!is.na(Nosjp))
...
rujukan <- data %>%
  subset(Nmjnspulang != "Rujuk") %>%
  select(Nokapst,Kdppkperujuk,Kdppklayan,Nosjp) %>%
  unique()

%>%
  rename(Kdppklayan = Kdppkperujuk)

left_join(df1, df2, by=c('x1'='x2', 'y1'='y2'))



noka <- noka[order(noka$Nokapst,noka$Tglplgsjp),]
noka <- noka %>% group_by(Nokapst) %>%
  mutate(keterangan = lag(Nmjnspulang, n=1)) %>%
  mutate(flag = paste0(as.character(keterangan)," - ",
                        as.character(Nmjnspulang)))
noka <- noka %>%
  filter(str_detect(flag,"Rujuk")) %>%
  select(-keterangan,-flag)
  
noka$flag <- with(noka,
                  ifelse(lag(Nokapst) == Nokapst | lead(Nokapst) == Nokapst,
                         "bener","salah"))
noka <- noka %>% subset(flag == "bener") %>%
  select(-flag)
noka <- noka %>% group_by(Nokapst) %>%
  mutate(Nmppkperujuk1 = lag(Nmppklayan, n=1))

noka <- noka %>%
  subset(Nmppkperujuk1 != Nmppklayan | is.na(Nmppkperujuk1))
noka$flag <- with(noka,
                  ifelse(lag(Nokapst) == Nokapst | lead(Nokapst) == Nokapst,
                         "bener","salah"))
noka <- noka %>% subset(flag == "bener") %>%
  select(-flag,-Nmppkperujuk1)

noka <- noka[order(noka$Nokapst,noka$Tglplgsjp),]
noka <- noka %>% group_by(Nokapst) %>%
  mutate(tgl_before = lag(Tglplgsjp, n=1))

noka$jeda <- as.integer(noka$Tgldtgsjp) - as.integer(noka$tgl_before)
noka$jeda <- as.numeric(noka$jeda) %>%
  replace(is.na(.),0)
noka <- noka %>% subset(jeda < 7)
...
write.csv(noka, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_rujuk.csv",
          na="", row.names = FALSE)
...
noka$flag1 <- with(noka,
                  ifelse(lag(Nmppklayan) != Nmppklayan | lead(Nmppklayan) != Nmppklayan,
                         "bener","salah"))
noka <- noka %>% subset(flag1 == "bener")
noka$flag <- with(noka,
                  ifelse(lag(Nokapst) == Nokapst | lead(Nokapst) == Nokapst,
                         "bener","salah"))
noka <- noka %>% subset(flag == "bener")
...
noka$jeda <- as.numeric(noka$jeda) %>%
  replace(is.na(.), "")
noka$jeda <- as.factor(noka$jeda)

noka$flag2 <- with(noka,
                  ifelse(lag(Tglplgsjp) == Tgldtgsjp | lead(Tgldtgsjp) == Tglplgsjp,
                         "bener","salah"))
...
%>%
  ...
  filter(str_detect(Diagnosis,"A099")) %>%
  select(-Diagnosis,-Nmtkp)

a099ranap <- a099ranap %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
a099ranap <- a099ranap %>% mutate(Diagsekunder = na_if(Diagsekunder, ""))
a099ranap <- a099ranap %>% mutate(Procedure = na_if(Procedure, "-"))
a099ranap <- a099ranap %>% mutate(Procedure = na_if(Procedure, ""))

a099ranap$Flag_1 <- with(a099ranap,
                         ifelse(Kddiagprimer == "A099", "Utama","Penyulit"))

a099ranap <- a099ranap %>%
  mutate(
    Kddiagprimer1 = case_when(
      Kddiagprimer == "A099" ~ NA,
      ...
      TRUE ~ Kddiagprimer))
a099ranap$Nmdiagprimer1 <- with(a099ranap,
                                ifelse(is.na(a099ranap$Kddiagprimer1),NA,Nmdiagprimer))
a099ranap <- a099ranap %>%
  mutate(Diagprimer = paste0(as.character(Kddiagprimer1)," - ",
                             as.character(Nmdiagprimer1)))
a099ranap$Diagprimer <- as.character(trimws(gsub("NA - NA",NA,a099ranap$Diagprimer)), "both")
a099ranap <- a099ranap %>%
  mutate(Diagnosa = paste0(as.character(Diagprimer),";",
                           as.character(Diagsekunder)))
a099ranap$Diagnosa <- as.character(trimws(gsub(";NA|NA;|NA","",a099ranap$Diagnosa)), "both")

a099ranap <- a099ranap %>% select(-Kddiagprimer1,-Nmdiagprimer1,-Diagprimer)

a099ranap <- a099ranap %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
a099ranap <- a099ranap %>% mutate(Diagnosa = na_if(Diagnosa, ""))
a099ranap <- a099ranap %>% mutate(Procedure = na_if(Procedure, "-"))
a099ranap$Diagnosa <- as.character(trimws(a099ranap$Diagnosa), "both")

library(splitstackshape)
a099ranap1 <- concat.split(a099ranap, "Diagnosa", ";")
colnames(a099ranap1)

a099ranap1$Diagnosa_1 <- as.character(trimws(a099ranap1$Diagnosa_1))
a099ranap1$Diagnosa_2 <- as.character(trimws(a099ranap1$Diagnosa_2))
a099ranap1$Diagnosa_3 <- as.character(trimws(a099ranap1$Diagnosa_3))
a099ranap1$Diagnosa_4 <- as.character(trimws(a099ranap1$Diagnosa_4))
a099ranap1$Diagnosa_5 <- as.character(trimws(a099ranap1$Diagnosa_5))
a099ranap1$Diagnosa_6 <- as.character(trimws(a099ranap1$Diagnosa_6))
a099ranap1$Diagnosa_7 <- as.character(trimws(a099ranap1$Diagnosa_7))
a099ranap1$Diagnosa_8 <- as.character(trimws(a099ranap1$Diagnosa_8))
a099ranap1$Diagnosa_9 <- as.character(trimws(a099ranap1$Diagnosa_9))

a099ranap1$Diagnosa_1 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_1),NA,Diagnosa_1))
a099ranap1$Diagnosa_2 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_2),NA,Diagnosa_2))
a099ranap1$Diagnosa_3 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_3),NA,Diagnosa_3))
a099ranap1$Diagnosa_4 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_4),NA,Diagnosa_4))
a099ranap1$Diagnosa_5 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_5),NA,Diagnosa_5))
a099ranap1$Diagnosa_6 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_6),NA,Diagnosa_6))
a099ranap1$Diagnosa_7 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_7),NA,Diagnosa_7))
a099ranap1$Diagnosa_8 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_8),NA,Diagnosa_8))
a099ranap1$Diagnosa_9 <- with(a099ranap1,ifelse(is.na(a099ranap1$Diagnosa_9),NA,Diagnosa_9))

a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_01 = case_when(
      Diagnosa_1 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_02 = case_when(
      Diagnosa_2 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_03 = case_when(
      Diagnosa_3 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_04 = case_when(
      Diagnosa_4 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_05 = case_when(
      Diagnosa_5 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_06 = case_when(
      Diagnosa_6 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_07 = case_when(
      Diagnosa_7 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_08 = case_when(
      Diagnosa_8 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
...
a099ranap1 <- a099ranap1 %>%
  mutate(
    Diagnosa_09 = case_when(
      Diagnosa_9 == "A099 - Gastroenteritis and colitis of unspecified origin" ~ NA,
      Diagnosa_9 == "Z039 - Observation for suspected disease or condition, unspecified" ~ NA,
      Diagnosa_9 == "Z041 - Examination and observation following transport accident" ~ NA,
      Diagnosa_9 == "Z359 - Supervision of high-risk pregnancy, unspecified" ~ NA,
      Diagnosa_9 == "Z491 - Extracorporeal dialysis" ~ NA,
      Diagnosa_9 == "Z501 - Other physical therapy" ~ NA,
      Diagnosa_9 == "Z719 - Counselling, unspecified" ~ NA,
      Diagnosa_9 == "Z088 - Follow-up examination after other treatment for malignant neoplasm" ~ NA,
      Diagnosa_9 == "Z089 - Follow-up examination after unspecified treatment for malignant neoplasm" ~ NA,
      Diagnosa_9 == "Z090 - Follow-up examination after surgery for other conditions" ~ NA,
      Diagnosa_9 == "Z091 - Follow-up examination after radiotherapy for other conditions" ~ NA,
      Diagnosa_9 == "Z092 - Follow-up examination after chemotherapy for other conditions" ~ NA,
      Diagnosa_9 == "Z093 - Follow-up examination after psychotherapy" ~ NA,
      Diagnosa_9 == "Z094 - Follow-up examination after treatment of fracture" ~ NA,
      Diagnosa_9 == "Z097 - Follow-up examination after combined treatment for other conditions" ~ NA,
      Diagnosa_9 == "Z098 - Follow-up examination after other treatment for other conditions" ~ NA,
      Diagnosa_9 == "Z099 - Follow-up examination after unspecified treatment for other conditions" ~ NA,
      Diagnosa_9 == "Z340 - Supervision of normal first pregnancy" ~ NA,
      Diagnosa_9 == "Z349 - Supervision of normal pregnancy, unspecified" ~ NA,
      Diagnosa_9 == "Z504 - Psychotherapy, not elsewhere classified" ~ NA,
      Diagnosa_9 == "Z509 - Care involving use of rehabilitation procedure, unspecified" ~ NA,
      Diagnosa_9 == "Z549 - Convalescence following unspecified treatment" ~ NA,
      Diagnosa_9 == "Z898 - Acquired absence of upper and lower limbs [any level]" ~ NA,
      Diagnosa_9 == "Z908 - Acquired absence of other organs" ~ NA,
      Diagnosa_9 == "Z961 - Presence of intraocular lens" ~ NA,
      Diagnosa_9 == "Z988 - Other specified postsurgical states" ~ NA,
      Diagnosa_9 == "Z489 - Surgical follow-up care, unspecified" ~ NA,
      TRUE ~ Diagnosa_9)) %>%
  select(-Diagnosa_9)
...
a099ranap1$Diagakhir <- as.character(trimws(gsub(";NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA|;NA;NA;NA;NA|;NA;NA;NA|;NA;NA|;NA|NA;|NA","",a099ranap1$Diagakhir)),"both")
a099ranap1 <- a099ranap1 %>%
  select(-Diagnosa_01,-Diagnosa_02,-Diagnosa_03,-Diagnosa_04,-Diagnosa_05,
         -Diagnosa_06,-Diagnosa_07,-Diagnosa_08,-Diagnosa_09,-Diagnosa)
a099ranap1 <- a099ranap1 %>% mutate(Diagakhir = na_if(Diagakhir, ""))
a099ranap1 <- a099ranap1 %>% rename(Diagnosa = Diagakhir)
...
a099ranap1$Procedure <- as.character(trimws(a099ranap1$Procedure), "both")
a099ranap1 <- concat.split(a099ranap1, "Procedure", ";")
colnames(a099ranap1)

a099ranap1$Procedure_1 <- as.character(trimws(a099ranap1$Procedure_1))
a099ranap1$Procedure_2 <- as.character(trimws(a099ranap1$Procedure_2))
a099ranap1$Procedure_3 <- as.character(trimws(a099ranap1$Procedure_3))
a099ranap1$Procedure_4 <- as.character(trimws(a099ranap1$Procedure_4))
a099ranap1$Procedure_5 <- as.character(trimws(a099ranap1$Procedure_5))
a099ranap1$Procedure_6 <- as.character(trimws(a099ranap1$Procedure_6))
a099ranap1$Procedure_7 <- as.character(trimws(a099ranap1$Procedure_7))
a099ranap1$Procedure_8 <- as.character(trimws(a099ranap1$Procedure_8))
a099ranap1$Procedure_9 <- as.character(trimws(a099ranap1$Procedure_9))

a099ranap1$Procedure_1 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_1),NA,Procedure_1))
a099ranap1$Procedure_2 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_2),NA,Procedure_2))
a099ranap1$Procedure_3 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_3),NA,Procedure_3))
a099ranap1$Procedure_4 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_4),NA,Procedure_4))
a099ranap1$Procedure_5 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_5),NA,Procedure_5))
a099ranap1$Procedure_6 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_6),NA,Procedure_6))
a099ranap1$Procedure_7 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_7),NA,Procedure_7))
a099ranap1$Procedure_8 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_8),NA,Procedure_8))
a099ranap1$Procedure_9 <- with(a099ranap1,ifelse(is.na(a099ranap1$Procedure_9),NA,Procedure_9))
...
a099ranap1$Procedure_new <- as.character(trimws(gsub("NA;NA;NA;NA;NA;NA;NA;NA;NA|NA;NA;NA;NA;NA;NA;NA;NA|NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA|;NA;NA;NA;NA|;NA;NA;NA|;NA;NA|;NA|NA",
                                                   "",a099ranap1$Procedure_new)),"both")
a099ranap1 <- a099ranap1 %>%
  select(-Procedure,-Procedure_1,-Procedure_2,-Procedure_3,-Procedure_4,-Procedure_5,-Procedure_6,-Procedure_7,-Procedure_8,-Procedure_9)
...
a099ranap1 <- a099ranap1 %>% mutate(Diagnosa = na_if(Diagnosa, ""))
a099ranap1 <- a099ranap1 %>% mutate(Procedure_new = na_if(Procedure_new, ""))

a099ranap1 <- a099ranap1[order(a099ranap1$Nokapst,a099ranap1$Tglplgsjp),]
a099ranap1 <- a099ranap1 %>% group_by(Nokapst) %>%
  mutate(tgl_before = lag(Tglplgsjp, n=1))
a099ranap1$jeda <- as.integer(a099ranap1$Tgldtgsjp) - as.integer(a099ranap1$tgl_before)
a099ranap1$jeda <- as.numeric(a099ranap1$jeda)
...
a099ranap1$kunjung_ke <- abs(a099ranap1$kunjung_ke )
...
write.csv(data, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_n140.csv",
          na="", row.names = FALSE)
