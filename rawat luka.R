...
datari <- rbind(df_jan19, df_feb19, df_mar19, df_apr19, df_mei19, df_jun19,
                df_jul19, df_agu19, df_sep19, df_okt19, df_nov19, df_des19,
                df_jan20, df_feb20, df_mar20, df_apr20, df_mei20, df_jun20,
                df_jul20, df_agu20, df_sep20, df_okt20, df_nov20, df_des20,
                df_jan21, df_feb21, df_mar21, df_apr21, df_mei21, df_jun21,
...
datari$Tgldtgsjp <- as.Date(datari$Tgldtgsjp, format = "%m/%d/%Y")
datari$Tglplgsjp <- as.Date(datari$Tglplgsjp, format = "%m/%d/%Y")
...
data <- left_join(datanokapst, datari, by = "Nokapst")
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>%
  group_by(Nokapst) %>%
  mutate(lastcbg = lag(Kdinacbgs))
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>%
  group_by(Nokapst) %>%
  mutate(nextcbg = lead(Kdinacbgs))
data$cbg <- paste0(as.character(data$lastcbg),"-",
                   as.character(data$Kdinacbgs),"-",
                   data$nextcbg)
rm(datanokapst,datari)

data0 <- data %>%
  filter(str_detect(cbg,c("Z-3-27-0")))
data0 <- data0[order(data0$Nokapst,data0$Tglplgsjp),]

#mencari pasien usg karena persalinan

datausgpersalinan <- datausgpersalinan %>% drop_na(Nosjp)
datausgpersalinannoka <- datausgpersalinan %>% select(Nokapst)
datausgpersalinannoka <- datausgpersalinannoka[!(duplicated(datausgpersalinannoka) |
                                                   duplicated(datausgpersalinannoka, fromLast = TRUE)), ]

#mencari pasien usg pada data persalinan (persalinan saja)
datapersalinanandusg <- left_join(datausgpersalinannoka, data,
                                  by = "Nokapst")

#menggabungkan persalinan & usg
datafinal <- rbind(datapersalinanandusg,datausgpersalinan)
datafinal <- datafinal[order(datafinal$Nokapst,
                             datafinal$Tglplgsjp),]

rm(datapersalinanandusg,datanokapst,datausgpersalinan,datausgpersalinannoka)
...
write.xlsx(datafinal, file = "Persalinan_USG.xlsx")
...
data <- data %>% subset(Nmtkp == "RITL")

data <- data %>% select(Nokapst,Nosjp,Jkpst,Umur,NmFKRTL,Tgldtgsjp,
...
write.xlsx(data, file = "Lamongan_P39.xlsx")
