...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  ...
  filter(str_detect(Diagnosa,c("K219"))) %>%
  filter(!str_detect(Procedure,c("4412|4413|4414|4511|4516|4525|
                                4224|4311|4513|4523|endoscop"))) %>%
  select(-Diagnosa)
...
data22 <- data %>% subset(Tglpelayanan >= "2022-01-01")
...
data$Keterangan <- with(data,
                        ifelse(str_detect(Nmdiagprimer, c("Pneumonia|pneumonia")),"Diagnosa Utama",
                                          "Diagnosa Sekunder"))
...
write.xlsx(data22, file = "K219_tanpa_endoskopi.xlsx")
