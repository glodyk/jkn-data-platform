...
ked_21 <- read_csv("Sheet 1_Full Data_data (4).csv")
ked_22 <- read_csv("Sheet 1_Full Data_factprosedur_2014_18.csv")
ked_23 <- read_csv("Sheet 1_Full Data_data (3).csv")
ked_22 <- read_csv("Sheet 1_Full factklaim_hist 2023_tglplgsjp.csv")
...
data19 <- read_csv("Sheet 1_Full Data_kediri 2019.csv") %>%
  ...
  filter(str_detect(Diagnosa,c("J96")))
...
data25 <- read_csv("Sheet 1_Full Data_kediri (des2025).csv") %>%
  select (Nokapst,Umur,Jkpst,,Kelashak,Klsrawat,Nmdati2Layan,Jenisppkperujuk,
          Typeppkperujuk,Nmppkperujuk,Kdppklayan,Kelasrsmenkes,Nosjp,Nmtkp,
          Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Tglreg,Tglstjkeu,Kdinacbgs,
          Nminacbgs,kdsa,kdsd,kdsi,kdsp,kdsr,deskripsisd,deskripsisi,deskripsisp,
  ...
  mutate(Diagnosa = paste0(as.character(Kddiagprimer)," - ",
                           ...
                           as.character(Diagsekunder))) %>%
  filter(str_detect(Diagnosa,c("J96")))
data25 <- data25 %>% mutate(kdsa = na_if(kdsa, "None"))
data25 <- data25 %>% mutate(kdsd = na_if(kdsd, "None"))
data25 <- data25 %>% mutate(kdsi = na_if(kdsi, "None"))
data25 <- data25 %>% mutate(kdsp = na_if(kdsp, "None"))
data25 <- data25 %>% mutate(kdsr = na_if(kdsr, "None"))
data25 <- data25 %>% mutate(kdsa = na_if(kdsa, "NONE"))
data25 <- data25 %>% mutate(kdsd = na_if(kdsd, "NONE"))
data25 <- data25 %>% mutate(kdsi = na_if(kdsi, "NONE"))
data25 <- data25 %>% mutate(kdsp = na_if(kdsp, "NONE"))
data25 <- data25 %>% mutate(kdsr = na_if(kdsr, "NONE"))
data25 <- data25 %>% mutate(deskripsisd = na_if(deskripsisd, "-"))
data25 <- data25 %>% mutate(deskripsisi = na_if(deskripsisi, "-"))
data25 <- data25 %>% mutate(deskripsisp = na_if(deskripsisp, "-"))
data25 <- data25 %>% mutate(deskripsisr = na_if(deskripsisr, "-"))

data <- rbind(data19,data20,data21,data22,data23,data24,datamei24,data25)
rm(data19,data20,data21,data22,data23,data24,datamei24,data25)
...
data$klasifikasi <- with(data,
                      ifelse(str_detect(Diagnosa,c("J96")),
                                        "Gagal napas",
                                        "Gangguan nafas paska prosedur"))
data$klasifikasi <- with(data,
                         ifelse(str_detect(Kddiagprimer,c("J96")),
                                "Diagnosa Utama",
                                "Diagnosa Sekunder"))
data$Ventilator <- with(data,
                         ifelse(str_detect(Procedure,c("9671|9672")),
                                "Terpasang Ventilator",
                                "Tanpa Ventilator"))
data$Ventilator <- with(data,
                        ifelse(is.na(data$Ventilator),
                               "Tanpa Ventilator",Ventilator))
...
datakoreksi <- data %>%
  ...
  subset(Ventilator == "Tanpa Ventilator")

write.csv(data,
          "D://data gresik//MTF KC GRESIK//gresik_2020//J95_j96.csv",
          na="", row.names = FALSE)

write.xlsx(datakoreksi, file = "j96_koreksi.xlsx")

write.xlsx(data, file = "J95_j96.xlsx")
...
ceknoka <- data %>% 
  subset(Nmtkp == "RJTL") %>%
  subset(Nokapst %in% c("313339249","448806042","5313758"))
ceknoka <- ceknoka[order(ceknoka$Nokapst,ceknoka$Tglplgsjp),]
#==========================================================================================
cek1 <- data %>%
...
#==============================================================================
save(df_data, file="df_data_smntr.rda")
load("df_data_smntr.rda")
#==============================================================================
setwd("D:/data gresik/MTF KC GRESIK/gresik_2020/tmp")
load("df22_6pintu.rda")

df22 <- df2 %>%
  select(Nosjp,Norjkawalsep,Sumber,keterangan)
#==============================================================================
...
data$Diagnosa <- as.character(trimws(gsub("NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA|;NA;NA;NA;NA|;NA;NA;NA|;NA;NA|;NA|NA;|NA","",data$Diagnosa)),"both")
data <- data %>% mutate(Diagnosa = na_if(Diagnosa, ""))
...
data$Flag <- with(data,
                  ifelse(Diagflag == "NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA",
                         "Non Spesialistik", "Spesialistik"))
data$Diagflag <- as.character(trimws(gsub("NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA|;NA;NA;NA;NA|;NA;NA;NA|;NA;NA|;NA|NA;|NA","",data$Diagflag)),"both")
