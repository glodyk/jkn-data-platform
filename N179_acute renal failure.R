...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmppklayan,Tgldtgsjp,
          ...
          Severitylevel) %>%
  filter(str_detect(Diagsekunder,c("179"))) %>%
  filter(str_detect(Severitylevel,c("III"))) %>%
...
data$keterangan <- "N179 diganti N19 - Unspecified kidney failure"
...
write.xlsx(data, file = "N179 diganti N19 Unspecified kidney failure.xlsx")
