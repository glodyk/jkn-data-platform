...
data21 <- read_csv("Sheet 1_Full Data gresik 2021.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Politujsjp,
          Kelasrsmenkes,Nmdati2Layan,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>%
  subset(Nmtkp == "RJTL") %>%
  subset(Kdppklayan == "0205R011") %>%
  subset(Kdinacbgs == "M-3-11-0")
...
write.xlsx(data, file = "M-3-11-0.xlsx")
