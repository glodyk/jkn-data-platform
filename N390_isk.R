...
data14 <- read_csv("Sheet 1_Full Data gresik 2014.csv") %>%
  ...
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Diagnosa,c("N390"))) %>%
  select(-Diagnosa)
...
datamei24 <- read_csv("Sheet 1_Full Data gresik mei_jul 2024.csv") %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Nmdati2Layan,Kdppklayan,
          ...
          Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>%
  filter(str_detect(Procedure,c("8349")))


%>%
  filter(str_detect(Procedure,c("5411|4709|6901|6902|8349|863")))
...
data <- rbind(data14,data15,data16,data17,data18,data19,data20,data21,
...
data <- data[order(data$Nosjp, data$Tglpelayanan),]
...
write.csv(data, "D://data gresik//MTF KC GRESIK//gresik_2020//N390 ranap.csv",
          na="", row.names = FALSE)

setwd("D:/Downloads")
library(excel.link)

spi <- xl.read.file("Top33_Coding_GRESIK.xlsx",
                    header = TRUE,
                    row.names = NULL,
                    col.names = NULL,
                    xl.sheet = "Lembar1",
                    ...
                    na = "",
                    excel.visible = FALSE) %>%
  subset(Judul == "20. Diagnosa Sekunder I49.3") %>%
  select(NOSEP) %>%
  rename(Nosjp = NOSEP)
spi <- spi %>% distinct()
spi$Keterangan <- "SPI Tahap I"

data <- left_join(data, spi, by = "Nosjp")
data <- data[order(data$Nosjp, data$Tglplgsjp),]
...
data1 <- data %>%
  ...
  subset(Nmppklayan == "RS Semen Gresik")

data1$Keterangan <- with(data1,
                         ifelse(is.na(data1$Keterangan), "Tambahan KC Gresik",
                                data1$Keterangan))

data1$Kdsevel <- as.character(trimws(substr(data1$Kdinacbgs,8,10)), "both")
data1 <- data1 %>%
  mutate(
    Sevel = case_when(
      Kdsevel == "0" ~ "0",
...
data1 <- data1 %>% subset(Sevel %in% c("3"))
...
write.xlsx(data1, file = "Diagnosa Sekunder I49.3 RS Semen.xlsx")
#==================================================================

data22$Sevel <- substr(data$Kdinacbgs, 8, 10)
