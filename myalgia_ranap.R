...
retina <- data %>%
  ...
  filter(str_detect(Procedure,"1434")) %>%
  select(Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nmtkp,Nosjp,Politujsjp,
  Tglpelayanan,Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,
  Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi)

retina <- retina[order(retina$Tglplgsjp),]
write.xlsx(retina, file = "Repair of retinal tear by laser photocoagulation.xlsx")

%>%
  filter(str_detect(Diagnosis,"H330")) 
...
a099ranap <- data %>%
  ...
  mutate(Diagnosis = paste0(as.character(Kddiagprimer)," - ",
                           as.character(Nmdiagprimer),";",
                           as.character(Diagsekunder))) %>%
  filter(str_detect(Diagnosis,"M79|M60")) %>%
  filter(str_detect(Kddiagprimer,"M7")) %>%
...
a099ranap <- a099ranap[order(a099ranap$Tglplgsjp),]
write.xlsx(a099ranap, file = "Myalgia_ranap.xlsx")

dipisah operative -> 3 digit oertama > 870 maka nonoperative
...
a099ranap <- data %>%
  ...
  mutate(Diagnosis = paste0(as.character(Kddiagprimer)," - ",
                            as.character(Nmdiagprimer),";",
                            as.character(Diagsekunder))) %>%
  filter(str_detect(Diagnosis,"A91|A010|A90")) %>%
  subset(Flag == "Non Spesialistik") %>%
  subset(kel_umur %in% c("12-16 Tahun","17-25 Tahun","26-35 Tahun","36-45 Tahun","46-55 Tahun","56-65 Tahun")) %>%
  select(Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nmtkp,Nosjp,Politujsjp,
         Tglpelayanan,Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,
         Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,Nmjnspulang,biayars,
         Biayaverifikasi)

a099ranap <- a099ranap %>% subset(Tglpelayanan >= "2024-01-01")
a099ranap <- a099ranap[order(a099ranap$Tglplgsjp),]
a099ranap$Flag <- with(a099ranap,
                       ifelse(str_detect(Kddiagprimer,c("A099|A010|A90")), "Utama","Penyulit"))
write.xlsx(a099ranap, file = "A90_A91_A010_ranap.xlsx")
...
write.csv(a099ranap1, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_A099.csv",
          na="", row.names = FALSE)
