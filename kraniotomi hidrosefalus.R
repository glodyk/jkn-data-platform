...
data15 <- read_csv("Sheet 1_Full Data gresik 2015.csv") %>%
  ...
  filter(str_detect(Diagnosa,c("Hydrocephalus|hydrocephalus"))) %>%
  filter(str_detect(Procedure,c("craniotomy|craniectomy|entriculostomy|0128|0159|0151|0206"))) %>%
  select(-Diagnosa)
...
kraniotomisjp <- kraniotomi %>% select(Nosjp)
kraniotomisjp$Keterangan <- "audit anomali 7"

data3 <- left_join(data,kraniotomisjp, by = c("Nosjp"="Nosjp"))
data3 <- data3[order(data3$Tglplgsjp),]
