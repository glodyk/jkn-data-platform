...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  filter(str_detect(Procedure,c("9765")))
...
#mencari pasien yg kuret
datanokapst <- data %>%
  dplyr::group_by(Nokapst)
datanokapst <- datanokapst %>% select(Nokapst)

#mencari pasien usg karena kuret
datausgkuret <- left_join(datanokapst, datausg, by = "Nokapst")
datausgkuret <- datausgkuret %>% drop_na(Nosjp)
datausgkuretnoka <- datausgkuret %>% select(Nokapst)
datausgkuretnoka <- datausgkuretnoka[!(duplicated(datausgkuretnoka) |
                     duplicated(datausgkuretnoka, fromLast = TRUE)), ]

#mencari pasien usg pada data kuret (kuret saja)
datakuretandusg <- left_join(datausgkuretnoka, data, 
                           by = "Nokapst")

#menggabungkan kuret & usg
datafinal <- rbind(datakuretandusg,datausgkuret)
datafinal <- datafinal[order(datafinal$Nokapst,
                             datafinal$Tglplgsjp),]

rm(datakuretandusg,datanokapst,datausgkuret,datausgkuretnoka)
...
datafinal <- datafinal %>%
  filter(str_detect(keterangan3,c("Kuretase;NA|USG;Kuretase")))
...
write.xlsx(datafinal, file = "Kuretase_USG.xlsx")
