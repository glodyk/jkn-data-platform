...
#========================================================================
hade <- data %>%
  filter(str_detect(Procedure,c("3995")))

hade <- hade %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
hade <- hade %>% mutate(Diagsekunder = na_if(Diagsekunder, ""))
hade <- hade %>% mutate(Procedure = na_if(Procedure, "-"))
hade <- hade %>% mutate(Procedure = na_if(Procedure, ""))

hade <- hade[order(hade$Nokapst,hade$Tglplgsjp),]
hade <- hade %>% group_by(Nokapst) %>%
  mutate(tgl_before = lag(Tglplgsjp, n=1))
hade$jeda <- as.integer(hade$Tgldtgsjp) - as.integer(hade$tgl_before)
hade$jeda <- as.numeric(hade$jeda)

hade <- hade[order(hade$Nokapst,hade$Tgldtgsjp),]
hade <- hade %>%
  group_by(Nokapst) %>% 
  mutate(pertama = first(Tgldtgsjp))
hade$seminggu <- as.Date(hade$pertama) + 7
...
hade$kunjung_ke <- abs(hade$kunjung_ke )
#========================================================================
write.csv(hade, "D:/data gresik/MTF KC GRESIK/gresik_2020/db hemodialisa.csv",
          na="", row.names = FALSE)
