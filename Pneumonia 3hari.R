...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  filter(str_detect(Procedure,c("541")),
         str_detect(Procedure,c("470")))

df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  select (Nokapst,Jkpst,Umur,Kelashak,Segmen,Nmdati2Terdaftar,Nmppkterdaftar,
          Flagspesialistik,Flagtacc,Sumberkunjungan,Nosjp,Tgldtgsjp,Tglplgsjp,
          Tglpelayanan,Nmdati2Layan,Jenisppklayan,Typeppklayan,Nmppklayan,
          Kelasrsmenkes,Nmtkp,Klsrawat,Politujsjp,Namadpjp,Nmdati2Perujuk,
          Nmppkperujuk,Jenisppkperujuk,Typeppkperujuk,Kddiagmasuk,
          Nmdiagmasuk,Nmcmg,Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,Nmjnspulang,Tarifgroup,biayars,Biayaverifikasi) %>% 
...
data$Diagnosa <- paste0(as.character(data$Diagprimer)," - ",
                        as.character(data$Diagsekunder))

data <- data %>%
  filter(str_detect(Diagnosa,c("J18"))) %>%
  subset(LOS <= 3) %>%
  filter(str_detect(Nmjnspulang,c("Meninggal|Rujuk")))

data <- data %>% select(Nokapst,Nosjp,Jkpst,Umur,NmFKRTL,Tgldtgsjp,
                        Tglplgsjp,LOS,Nmjnspulang,Namadpjp,Kdinacbgs,
                        Kddiagprimer,Nmdiagprimer,Diagsekunder,
                        Procedure,biayars,Biayaverifikasi)
data <- data[order(data$NmFKRTL, data$Tglplgsjp),]
write.xlsx(data, file = "Peneumonia 3 hari.xlsx")
