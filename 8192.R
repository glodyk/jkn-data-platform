...
data <- data %>% group_by(Nokapst) %>%
  mutate(interval = as.numeric(Tglplgsjp-lag(Tglplgsjp)))
...
data <- data %>%
  select (Nokapst,kunjungan,Nosjp,Jkpst,Nmtkp,Nmppklayan,Tgldtgsjp,
          Tglplgsjp,interval,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,Namadpjp,Nmjnspulang,biayars,
          Biayaverifikasi)
write.xlsx(data, file = "Injection Of Therapeutic Substance Into Joint Or Ligament.xlsx")
