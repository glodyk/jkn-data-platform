...
kediri8 <- read_csv("Sheet 1_Full Data_factkronis_kediri.csv")
colnames(kediri8)


kediri1 <- read_csv("Sheet 1_Full Data_data (1).csv")
kediri2 <- read_csv("Sheet 1_Full Data_data (2).csv")
kediri3 <- read_csv("Sheet 1_Full Data_data (3).csv") 
kediri4 <- read_csv("Sheet 1_Full Data_data (4).csv") 
kediri5 <- read_csv("Sheet 1_Full Data_data (nonkapnull_18).csv") 
kediri6 <- read_csv("Sheet 1_Full Data_data (6).csv") 
kediri7 <- read_csv("Sheet 1_Full Data_data (7).csv")
...
data26 <- read_csv("Sheet 1_Full Data_kediri (jan2026).csv") %>%
  select (Nokapst,Umur,Jkpst,,Kelashak,Klsrawat,Nmdati2Layan,Jenisppkperujuk,
          Typeppkperujuk,Nmppkperujuk,Kdppklayan,Kelasrsmenkes,Nosjp,Nmtkp,
          Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Tglreg,Tglstjkeu,Kdinacbgs,
          Nminacbgs,kdsa,kdsd,kdsi,kdsp,kdsr,deskripsisd,deskripsisi,deskripsisp,
          deskripsisr,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,tarifsa,tarifsd,tarifsi,tarifsp,tarifsr,biayars,Biayaverifikasi)
data26 <- data26 %>% mutate(kdsa = na_if(kdsa, "None"))
data26 <- data26 %>% mutate(kdsd = na_if(kdsd, "None"))
data26 <- data26 %>% mutate(kdsi = na_if(kdsi, "None"))
data26 <- data26 %>% mutate(kdsp = na_if(kdsp, "None"))
data26 <- data26 %>% mutate(kdsr = na_if(kdsr, "None"))
data26 <- data26 %>% mutate(kdsa = na_if(kdsa, "NONE"))
data26 <- data26 %>% mutate(kdsd = na_if(kdsd, "NONE"))
data26 <- data26 %>% mutate(kdsi = na_if(kdsi, "NONE"))
data26 <- data26 %>% mutate(kdsp = na_if(kdsp, "NONE"))
data26 <- data26 %>% mutate(kdsr = na_if(kdsr, "NONE"))
data26 <- data26 %>% mutate(deskripsisd = na_if(deskripsisd, "-"))
data26 <- data26 %>% mutate(deskripsisi = na_if(deskripsisi, "-"))
data26 <- data26 %>% mutate(deskripsisp = na_if(deskripsisp, "-"))
data26 <- data26 %>% mutate(deskripsisr = na_if(deskripsisr, "-"))

data26$Politujsjp <- data26$Politujsjp %>%
  modify_if(is.character, toupper)

data <- rbind(data19,data20,data21,data22,data23,data24,datamei24,data26)
rm(data19,data20,data21,data22,data23,data24,datamei24,data26)
...
#==========================================================================================
cek1 <- data %>%
  subset(Nosjp == "0210R0101022V002464")
cek1$Politujsjp <- "IGD"
cek2 <- data %>%
  subset(Nosjp == "0185R0040723V000003")
cek2$Politujsjp <- "HDL"
cek3 <- data %>%
  subset(Nosjp == "1313R0010724V005274")
cek3$Politujsjp <- "HIV"
...
dataeror <- data %>% 
  ...
  subset(Nosjp != "0185R0040723V000003") %>%
  subset(Nosjp != "1313R0010724V005274")
data <- rbind(dataeror,cek1,cek2,cek3)
cek <- data %>% 
  subset(Nmtkp == "RJTL") %>%
  subset(is.na(Politujsjp))
rm(cek,cek1,cek2,cek3,dataeror)
#======================================================================================
...
df2 <- df2 %>% select(Nokapst,Nosjp,Norjkawalsep,Sumber,tgl_before,selisih_hari,
                      keterangan,Norjk_ke)
#=========================================================================
df1 <- left_join(df2,df_ruj, by = c("Norjkawalsep"="no_kunjungan"))
cek <- df1 %>%
...
cek <- data %>% subset(Nokapst.x != Nokapst.y)
rm(cek)
data <- data %>%
  rename(Nokapst = Nokapst.x)

data <- data %>% select(-nokapst,-Nokapst.y)
rm(df1,df_data)
#====================================================================================
...
#=======================================================================
datari <- datari %>% mutate(Diagsekunder = na_if(Diagsekunder, "-"))
datari <- datari %>% mutate(Procedure = na_if(Procedure, "-"))

save(datarj, file = "ur_rajal.rda")
save(datari, file = "ur_ranap.rda")
...
rm(datari,datarj)
data <- data %>% subset(Tglpelayanan >= "2021-01-01")
data <- data[order(data$Nokapst,data$Tglplgsjp),]
#=======================================================================
...
data <- data %>% select(-Diagprimer,-Kddiagprimer1,-Nmdiagprimer1)
...
data$Diagnosa_15 <- as.character(trimws(data$Diagnosa_15))
data$Diagnosa_16 <- as.character(trimws(data$Diagnosa_16))
...
data$Diagnosa_15 <- with(data,ifelse(is.na(data$Diagnosa_15),NA,Diagnosa_15))
data$Diagnosa_16 <- with(data,ifelse(is.na(data$Diagnosa_16),NA,Diagnosa_16))
...
data <- data %>%
  mutate(Diagnosa = paste0(as.character(Diagnosa_01),";",
                           ...
                           as.character(Diagnosa_15),";",
                           as.character(Diagnosa_16)))

data$Diagnosa <- as.character(trimws(gsub("NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA|;NA;NA;NA;NA|;NA;NA;NA|;NA;NA|;NA|NA;|NA","",data$Diagnosa)),"both")
data <- data %>% mutate(Diagnosa = na_if(Diagnosa, ""))
...
data <- data %>%
  mutate(
    Diagnosa_015 = case_when(
      Diagnosa_15 == "A010 - Typhoid fever" ~ NA,
      Diagnosa_15 == "A059 - Bacterial foodborne intoxication, unspecified" ~ NA,
      Diagnosa_15 == "A060 - Acute amoebic dysentery" ~ NA,
      Diagnosa_15 == "A150 - Tuberculosis of lung, confirmed by sputum microscopy with or without culture" ~ NA,
      Diagnosa_15 == "A184 - Tuberculosis of skin and subcutaneous tissue" ~ NA,
      Diagnosa_15 == "A270 - Leptospirosis icterohaemorrhagica" ~ NA,
      Diagnosa_15 == "A300 - Indeterminate leprosy" ~ NA,
      Diagnosa_15 == "A35 - Other tetanus" ~ NA,
      Diagnosa_15 == "A370 - Whooping cough due to Bordetella pertussis" ~ NA,
      Diagnosa_15 == "A46 - Erysipelas" ~ NA,
      Diagnosa_15 == "A510 - Primary genital syphilis" ~ NA,
      Diagnosa_15 == "A520 - Cardiovascular syphilis" ~ NA,
      Diagnosa_15 == "A540 - nococcal infection of lower genitourinary tract without periurethral or accessory gland abscess" ~ NA,
      Diagnosa_15 == "A90 - Dengue fever [classical dengue]" ~ NA,
      Diagnosa_15 == "A91 - Dengue haemorrhagic fever" ~ NA,
      Diagnosa_15 == "A920 - Chikungunya virus disease" ~ NA,
      Diagnosa_15 == "B001 - Herpesviral vesicular dermatitis" ~ NA,
      Diagnosa_15 == "B009 - Herpesviral infection, unspecified" ~ NA,
      Diagnosa_15 == "B019 - Varicella without complication" ~ NA,
      Diagnosa_15 == "B029 - Zoster without complication" ~ NA,
      Diagnosa_15 == "B059 - Measles without complication" ~ NA,
      Diagnosa_15 == "B07 - Viral warts" ~ NA,
      Diagnosa_15 == "B081 - Molluscum contagiosum" ~ NA,
      Diagnosa_15 == "B150 - Hepatitis A with hepatic coma" ~ NA,
      Diagnosa_15 == "B160 - Acute hepatitis B with delta-agent (coinfection) with hepatic coma" ~ NA,
      Diagnosa_15 == "B170 - Acute delta-(super)infection of hepatitis B carrier" ~ NA,
      Diagnosa_15 == "B171 - Acute hepatitis C" ~ NA,
      Diagnosa_15 == "B172 - Acute hepatitis E" ~ NA,
      Diagnosa_15 == "B24 - Unspecified human immunodeficiency virus [HIV] disease" ~ NA,
      Diagnosa_15 == "B260 - Mumps orchitis" ~ NA,
      Diagnosa_15 == "B350 - Tinea barbae and tinea capitis" ~ NA,
      Diagnosa_15 == "B351 - Tinea unguium" ~ NA,
      Diagnosa_15 == "B352 - Tinea manuum" ~ NA,
      Diagnosa_15 == "B353 - Tinea pedis" ~ NA,
      Diagnosa_15 == "B354 - Tinea corporis" ~ NA,
      Diagnosa_15 == "B356 - Tinea cruris" ~ NA,
      Diagnosa_15 == "B359 - Dermatophytosis, unspecified" ~ NA,
      Diagnosa_15 == "B360 - Pityriasis versicolor" ~ NA,
      Diagnosa_15 == "B500 - Plasmodium falciparum malaria with cerebral complications" ~ NA,
      Diagnosa_15 == "B510 - Plasmodium vivax malaria with rupture of spleen" ~ NA,
      Diagnosa_15 == "B520 - Plasmodium malariae malaria with nephropathy" ~ NA,
      Diagnosa_15 == "B530 - Plasmodium ovale malaria" ~ NA,
      Diagnosa_15 == "B54 - Unspecified malaria" ~ NA,
      Diagnosa_15 == "B650 - Schistosomiasis due to Schistosoma haematobium [urinary schistosomiasis]" ~ NA,
      Diagnosa_15 == "B680 - Von Willebrand disease" ~ NA,
      Diagnosa_15 == "B740 - Filariasis due to Wuchereria bancrofti" ~ NA,
      Diagnosa_15 == "B760 - Ancylostomiasis" ~ NA,
      Diagnosa_15 == "B769 - Hookworm disease, unspecified" ~ NA,
      Diagnosa_15 == "B770 - Ascariasis with intestinal complications" ~ NA,
      Diagnosa_15 == "B780 - Intestinal strongyloidiasis" ~ NA,
      Diagnosa_15 == "B850 - Pediculosis due to Pediculus humanus capitis" ~ NA,
      Diagnosa_15 == "B853 - Phthiriasis" ~ NA,
      Diagnosa_15 == "B86 - Scabies" ~ NA,
      Diagnosa_15 == "D179 - Benign lipomatous neoplasm, unspecified" ~ NA,
      Diagnosa_15 == "D500 - Iron deficiency anaemia secondary to blood loss (chronic)" ~ NA,
      Diagnosa_15 == "D649 - Anaemia, unspecified" ~ NA,
      Diagnosa_15 == "E100 - Insulin-dependent diabetes mellitus with coma" ~ NA,
      Diagnosa_15 == "E110 - Non-insulin-dependent diabetes mellitus with coma" ~ NA,
      Diagnosa_15 == "E119 - Non-insulin-dependent diabetes mellitus without complications" ~ NA,
      Diagnosa_15 == "E162 - Hypoglycaemia, unspecified" ~ NA,
      Diagnosa_15 == "E46 - Unspecified protein-energy malnutrition" ~ NA,
      Diagnosa_15 == "E569 - Vitamin deficiency, unspecified" ~ NA,
      Diagnosa_15 == "E610 - Copper deficiency" ~ NA,
      Diagnosa_15 == "E660 - Obesity due to excess calories" ~ NA,
      Diagnosa_15 == "E740 - Glycogen storage disease" ~ NA,
      Diagnosa_15 == "E785 - Hyperlipidaemia, unspecified" ~ NA,
      Diagnosa_15 == "E790 - Hyperuricaemia without signs of inflammatory arthritis and tophaceous disease" ~ NA,
      Diagnosa_15 == "F450 - Somatization disorder" ~ NA,
      Diagnosa_15 == "G430 - Migraine without aura [common migraine]" ~ NA,
      Diagnosa_15 == "G442 - Tension-type headache" ~ NA,
      Diagnosa_15 == "G470 - Disorders of initiating and maintaining sleep [insomnias]" ~ NA,
      Diagnosa_15 == "G510 - Bell's palsy" ~ NA,
      Diagnosa_15 == "H000 - Hordeolum and other deep inflammation of eyelid" ~ NA,
      Diagnosa_15 == "H010 - Blepharitis" ~ NA,
      Diagnosa_15 == "H020 - Entropion and trichiasis of eyelid" ~ NA,
      Diagnosa_15 == "H100 - Mucopurulent conjunctivitis" ~ NA,
      Diagnosa_15 == "H113 - Conjunctival haemorrhage" ~ NA,
      Diagnosa_15 == "H151 - Episcleritis" ~ NA,
      Diagnosa_15 == "H162 - Keratoconjunctivitis" ~ NA,
      Diagnosa_15 == "H524 - Presbyopia" ~ NA,
      Diagnosa_15 == "H531 - Subjective visual disturbances" ~ NA,
      Diagnosa_15 == "H600 - Abscess of external ear" ~ NA,
      Diagnosa_15 == "H612 - Impacted cerumen" ~ NA,
      Diagnosa_15 == "H669 - Otitis media, unspecified" ~ NA,
      Diagnosa_15 == "H810 - Meniere's disease" ~ NA,
      Diagnosa_15 == "I10 - Essential (primary) hypertension" ~ NA,
      Diagnosa_15 == "I839 - Varicose veins of lower extremities without ulcer or inflammation" ~ NA,
      Diagnosa_15 == "I840 - Internal thrombosed haemorrhoids" ~ NA,
      Diagnosa_15 == "I880 - Nonspecific mesenteric lymphadenitis" ~ NA,
      Diagnosa_15 == "J00 - Acute nasopharyngitis [common cold]" ~ NA,
      Diagnosa_15 == "J020 - Streptococcal pharyngitis" ~ NA,
      Diagnosa_15 == "J030 - Streptococcal tonsillitis" ~ NA,
      Diagnosa_15 == "J040 - Acute laryngitis" ~ NA,
      Diagnosa_15 == "J100 - Influenza with pneumonia, other influenza virus identified" ~ NA,
      Diagnosa_15 == "J120 - Adenoviral pneumonia" ~ NA,
      Diagnosa_15 == "J300 - Vasomotor rhinitis" ~ NA,
      Diagnosa_15 == "J304 - Allergic rhinitis, unspecified" ~ NA,
      Diagnosa_15 == "J340 - Abscess, furuncle and carbuncle of nose" ~ NA,
      Diagnosa_15 == "J40 - Bronchitis, not specified as acute or chronic" ~ NA,
      Diagnosa_15 == "J459 - Asthma, unspecified" ~ NA,
      Diagnosa_15 == "K210 - Gastro-oesophageal reflux disease with oesophagitis" ~ NA,
      Diagnosa_15 == "K291 - Other acute gastritis" ~ NA,
      Diagnosa_15 == "K297 - Gastritis, unspecified" ~ NA,
      Diagnosa_15 == "K30 - Dyspepsia" ~ NA,
      Diagnosa_15 == "K529 - Noninfective gastroenteritis and colitis, unspecified" ~ NA,
      Diagnosa_15 == "L010 - Impeti [any organism] [any site]" ~ NA,
      Diagnosa_15 == "L020 - Cutaneous abscess, furuncle and carbuncle of face" ~ NA,
      Diagnosa_15 == "L022 - Cutaneous abscess, furuncle and carbuncle of trunk" ~ NA,
      Diagnosa_15 == "L028 - Cutaneous abscess, furuncle and carbuncle of other sites" ~ NA,
      Diagnosa_15 == "L080 - Pyoderma" ~ NA,
      Diagnosa_15 == "L081 - Erythrasma" ~ NA,
      Diagnosa_15 == "L089 - Local infection of skin and subcutaneous tissue, unspecified" ~ NA,
      Diagnosa_15 == "L200 - Besnier's pruri" ~ NA,
      Diagnosa_15 == "L210 - Seborrhoea capitis" ~ NA,
      Diagnosa_15 == "L240 - Irritant contact dermatitis due to detergents" ~ NA,
      Diagnosa_15 == "L270 - Generalized skin eruption due to drugs and medicaments" ~ NA,
      Diagnosa_15 == "L271 - Localized skin eruption due to drugs and medicaments" ~ NA,
      Diagnosa_15 == "L272 - Dermatitis due to ingested food" ~ NA,
      Diagnosa_15 == "L300 - Nummular dermatitis" ~ NA,
      Diagnosa_15 == "L309 - Dermatitis, unspecified" ~ NA,
      Diagnosa_15 == "L42 - Pityriasis rosea" ~ NA,
      Diagnosa_15 == "L500 - Allergic urticaria" ~ NA,
      Diagnosa_15 == "L700 - Acne vulgaris" ~ NA,
      Diagnosa_15 == "L710 - Perioral dermatitis" ~ NA,
      Diagnosa_15 == "L732 - Hidradenitis suppurativa" ~ NA,
      Diagnosa_15 == "L743 - Miliaria, unspecified" ~ NA,
      Diagnosa_15 == "N12 - Tubulo-interstitial nephritis, not specified as acute or chronic" ~ NA,
      Diagnosa_15 == "N390 - Urinary tract infection, site not specified" ~ NA,
      Diagnosa_15 == "N47 - Redundant prepuce, phimosis and paraphimosis" ~ NA,
      Diagnosa_15 == "N508 - Other specified disorders of male genital organs" ~ NA,
      Diagnosa_15 == "N61 - Inflammatory disorders of breast" ~ NA,
      Diagnosa_15 == "N645 - Other signs and symptoms in breast" ~ NA,
      Diagnosa_15 == "N700 - Acute salpingitis and oophoritis" ~ NA,
      Diagnosa_15 == "N760 - Acute vaginitis" ~ NA,
      Diagnosa_15 == "N762 - Acute vulvitis" ~ NA,
      Diagnosa_15 == "N768 - Other specified inflammation of vagina and vulva" ~ NA,
      Diagnosa_15 == "O000 - Abdominal pregnancy" ~ NA,
      Diagnosa_15 == "O030 - Spontaneous abortion, incomplete, complicated by genital tract and pelvic infection" ~ NA,
      Diagnosa_15 == "O25 - Malnutrition in pregnancy" ~ NA,
      Diagnosa_15 == "O700 - First degree perineal laceration during delivery" ~ NA,
      Diagnosa_15 == "O701 - Second degree perineal laceration during delivery" ~ NA,
      Diagnosa_15 == "O702 - Third degree perineal laceration during delivery" ~ NA,
      Diagnosa_15 == "O703 - Fourth degree perineal laceration during delivery" ~ NA,
      Diagnosa_15 == "O800 - Spontaneous vertex delivery" ~ NA,
      Diagnosa_15 == "O801 - Spontaneous breech delivery" ~ NA,
      Diagnosa_15 == "O808 - Other single spontaneous delivery" ~ NA,
      Diagnosa_15 == "O809 - Single spontaneous delivery, unspecified" ~ NA,
      Diagnosa_15 == "O839 - Assisted single delivery, unspecified" ~ NA,
      Diagnosa_15 == "O921 - Cracked nipple associated with childbirth" ~ NA,
      Diagnosa_15 == "R040 - Epistaxis" ~ NA,
      Diagnosa_15 == "R560 - Febrile convulsions" ~ NA,
      Diagnosa_15 == "R51 - Headache" ~ NA,
      Diagnosa_15 == "T151 - Foreign body in conjunctival sac" ~ NA,
      Diagnosa_15 == "T171 - Foreign body in nostril" ~ NA,
      Diagnosa_15 == "T301 - Burn of first degree, body region unspecified" ~ NA,
      Diagnosa_15 == "T302 - Burn of second degree, body region unspecified" ~ NA,
      Diagnosa_15 == "T753 - Motion sickness" ~ NA,
      Diagnosa_15 == "T782 - Anaphylactic shock, unspecified" ~ NA,
      Diagnosa_15 == "T812 - Accidental puncture and laceration during a procedure, not elsewhere classified" ~ NA,
      Diagnosa_15 == "Z719 - Counselling, unspecified" ~ NA,
      Diagnosa_15 == "Z370 - Single live birth" ~ NA,
      TRUE ~ Diagnosa_15)) %>%
  select(-Diagnosa_15)

data <- data %>%
  mutate(
    Diagnosa_016 = case_when(
      Diagnosa_16 == "A010 - Typhoid fever" ~ NA,
      Diagnosa_16 == "A059 - Bacterial foodborne intoxication, unspecified" ~ NA,
      Diagnosa_16 == "A060 - Acute amoebic dysentery" ~ NA,
      Diagnosa_16 == "A150 - Tuberculosis of lung, confirmed by sputum microscopy with or without culture" ~ NA,
      Diagnosa_16 == "A184 - Tuberculosis of skin and subcutaneous tissue" ~ NA,
      Diagnosa_16 == "A270 - Leptospirosis icterohaemorrhagica" ~ NA,
      Diagnosa_16 == "A300 - Indeterminate leprosy" ~ NA,
      Diagnosa_16 == "A35 - Other tetanus" ~ NA,
      Diagnosa_16 == "A370 - Whooping cough due to Bordetella pertussis" ~ NA,
      Diagnosa_16 == "A46 - Erysipelas" ~ NA,
      Diagnosa_16 == "A510 - Primary genital syphilis" ~ NA,
      Diagnosa_16 == "A520 - Cardiovascular syphilis" ~ NA,
      Diagnosa_16 == "A540 - nococcal infection of lower genitourinary tract without periurethral or accessory gland abscess" ~ NA,
      Diagnosa_16 == "A90 - Dengue fever [classical dengue]" ~ NA,
      Diagnosa_16 == "A91 - Dengue haemorrhagic fever" ~ NA,
      Diagnosa_16 == "A920 - Chikungunya virus disease" ~ NA,
      Diagnosa_16 == "B001 - Herpesviral vesicular dermatitis" ~ NA,
      Diagnosa_16 == "B009 - Herpesviral infection, unspecified" ~ NA,
      Diagnosa_16 == "B019 - Varicella without complication" ~ NA,
      Diagnosa_16 == "B029 - Zoster without complication" ~ NA,
      Diagnosa_16 == "B059 - Measles without complication" ~ NA,
      Diagnosa_16 == "B07 - Viral warts" ~ NA,
      Diagnosa_16 == "B081 - Molluscum contagiosum" ~ NA,
      Diagnosa_16 == "B150 - Hepatitis A with hepatic coma" ~ NA,
      Diagnosa_16 == "B160 - Acute hepatitis B with delta-agent (coinfection) with hepatic coma" ~ NA,
      Diagnosa_16 == "B170 - Acute delta-(super)infection of hepatitis B carrier" ~ NA,
      Diagnosa_16 == "B171 - Acute hepatitis C" ~ NA,
      Diagnosa_16 == "B172 - Acute hepatitis E" ~ NA,
      Diagnosa_16 == "B24 - Unspecified human immunodeficiency virus [HIV] disease" ~ NA,
      Diagnosa_16 == "B260 - Mumps orchitis" ~ NA,
      Diagnosa_16 == "B350 - Tinea barbae and tinea capitis" ~ NA,
      Diagnosa_16 == "B351 - Tinea unguium" ~ NA,
      Diagnosa_16 == "B352 - Tinea manuum" ~ NA,
      Diagnosa_16 == "B353 - Tinea pedis" ~ NA,
      Diagnosa_16 == "B354 - Tinea corporis" ~ NA,
      Diagnosa_16 == "B356 - Tinea cruris" ~ NA,
      Diagnosa_16 == "B359 - Dermatophytosis, unspecified" ~ NA,
      Diagnosa_16 == "B360 - Pityriasis versicolor" ~ NA,
      Diagnosa_16 == "B500 - Plasmodium falciparum malaria with cerebral complications" ~ NA,
      Diagnosa_16 == "B510 - Plasmodium vivax malaria with rupture of spleen" ~ NA,
      Diagnosa_16 == "B520 - Plasmodium malariae malaria with nephropathy" ~ NA,
      Diagnosa_16 == "B530 - Plasmodium ovale malaria" ~ NA,
      Diagnosa_16 == "B54 - Unspecified malaria" ~ NA,
      Diagnosa_16 == "B650 - Schistosomiasis due to Schistosoma haematobium [urinary schistosomiasis]" ~ NA,
      Diagnosa_16 == "B680 - Von Willebrand disease" ~ NA,
      Diagnosa_16 == "B740 - Filariasis due to Wuchereria bancrofti" ~ NA,
      Diagnosa_16 == "B760 - Ancylostomiasis" ~ NA,
      Diagnosa_16 == "B769 - Hookworm disease, unspecified" ~ NA,
      Diagnosa_16 == "B770 - Ascariasis with intestinal complications" ~ NA,
      Diagnosa_16 == "B780 - Intestinal strongyloidiasis" ~ NA,
      Diagnosa_16 == "B850 - Pediculosis due to Pediculus humanus capitis" ~ NA,
      Diagnosa_16 == "B853 - Phthiriasis" ~ NA,
      Diagnosa_16 == "B86 - Scabies" ~ NA,
      Diagnosa_16 == "D179 - Benign lipomatous neoplasm, unspecified" ~ NA,
      Diagnosa_16 == "D500 - Iron deficiency anaemia secondary to blood loss (chronic)" ~ NA,
      Diagnosa_16 == "D649 - Anaemia, unspecified" ~ NA,
      Diagnosa_16 == "E100 - Insulin-dependent diabetes mellitus with coma" ~ NA,
      Diagnosa_16 == "E110 - Non-insulin-dependent diabetes mellitus with coma" ~ NA,
      Diagnosa_16 == "E119 - Non-insulin-dependent diabetes mellitus without complications" ~ NA,
      Diagnosa_16 == "E162 - Hypoglycaemia, unspecified" ~ NA,
      Diagnosa_16 == "E46 - Unspecified protein-energy malnutrition" ~ NA,
      Diagnosa_16 == "E569 - Vitamin deficiency, unspecified" ~ NA,
      Diagnosa_16 == "E610 - Copper deficiency" ~ NA,
      Diagnosa_16 == "E660 - Obesity due to excess calories" ~ NA,
      Diagnosa_16 == "E740 - Glycogen storage disease" ~ NA,
      Diagnosa_16 == "E785 - Hyperlipidaemia, unspecified" ~ NA,
      Diagnosa_16 == "E790 - Hyperuricaemia without signs of inflammatory arthritis and tophaceous disease" ~ NA,
      Diagnosa_16 == "F450 - Somatization disorder" ~ NA,
      Diagnosa_16 == "G430 - Migraine without aura [common migraine]" ~ NA,
      Diagnosa_16 == "G442 - Tension-type headache" ~ NA,
      Diagnosa_16 == "G470 - Disorders of initiating and maintaining sleep [insomnias]" ~ NA,
      Diagnosa_16 == "G510 - Bell's palsy" ~ NA,
      Diagnosa_16 == "H000 - Hordeolum and other deep inflammation of eyelid" ~ NA,
      Diagnosa_16 == "H010 - Blepharitis" ~ NA,
      Diagnosa_16 == "H020 - Entropion and trichiasis of eyelid" ~ NA,
      Diagnosa_16 == "H100 - Mucopurulent conjunctivitis" ~ NA,
      Diagnosa_16 == "H113 - Conjunctival haemorrhage" ~ NA,
      Diagnosa_16 == "H151 - Episcleritis" ~ NA,
      Diagnosa_16 == "H162 - Keratoconjunctivitis" ~ NA,
      Diagnosa_16 == "H524 - Presbyopia" ~ NA,
      Diagnosa_16 == "H531 - Subjective visual disturbances" ~ NA,
      Diagnosa_16 == "H600 - Abscess of external ear" ~ NA,
      Diagnosa_16 == "H612 - Impacted cerumen" ~ NA,
      Diagnosa_16 == "H669 - Otitis media, unspecified" ~ NA,
      Diagnosa_16 == "H810 - Meniere's disease" ~ NA,
      Diagnosa_16 == "I10 - Essential (primary) hypertension" ~ NA,
      Diagnosa_16 == "I839 - Varicose veins of lower extremities without ulcer or inflammation" ~ NA,
      Diagnosa_16 == "I840 - Internal thrombosed haemorrhoids" ~ NA,
      Diagnosa_16 == "I880 - Nonspecific mesenteric lymphadenitis" ~ NA,
      Diagnosa_16 == "J00 - Acute nasopharyngitis [common cold]" ~ NA,
      Diagnosa_16 == "J020 - Streptococcal pharyngitis" ~ NA,
      Diagnosa_16 == "J030 - Streptococcal tonsillitis" ~ NA,
      Diagnosa_16 == "J040 - Acute laryngitis" ~ NA,
      Diagnosa_16 == "J100 - Influenza with pneumonia, other influenza virus identified" ~ NA,
      Diagnosa_16 == "J120 - Adenoviral pneumonia" ~ NA,
      Diagnosa_16 == "J300 - Vasomotor rhinitis" ~ NA,
      Diagnosa_16 == "J304 - Allergic rhinitis, unspecified" ~ NA,
      Diagnosa_16 == "J340 - Abscess, furuncle and carbuncle of nose" ~ NA,
      Diagnosa_16 == "J40 - Bronchitis, not specified as acute or chronic" ~ NA,
      Diagnosa_16 == "J459 - Asthma, unspecified" ~ NA,
      Diagnosa_16 == "K210 - Gastro-oesophageal reflux disease with oesophagitis" ~ NA,
      Diagnosa_16 == "K291 - Other acute gastritis" ~ NA,
      Diagnosa_16 == "K297 - Gastritis, unspecified" ~ NA,
      Diagnosa_16 == "K30 - Dyspepsia" ~ NA,
      Diagnosa_16 == "K529 - Noninfective gastroenteritis and colitis, unspecified" ~ NA,
      Diagnosa_16 == "L010 - Impeti [any organism] [any site]" ~ NA,
      Diagnosa_16 == "L020 - Cutaneous abscess, furuncle and carbuncle of face" ~ NA,
      Diagnosa_16 == "L022 - Cutaneous abscess, furuncle and carbuncle of trunk" ~ NA,
      Diagnosa_16 == "L028 - Cutaneous abscess, furuncle and carbuncle of other sites" ~ NA,
      Diagnosa_16 == "L080 - Pyoderma" ~ NA,
      Diagnosa_16 == "L081 - Erythrasma" ~ NA,
      Diagnosa_16 == "L089 - Local infection of skin and subcutaneous tissue, unspecified" ~ NA,
      Diagnosa_16 == "L200 - Besnier's pruri" ~ NA,
      Diagnosa_16 == "L210 - Seborrhoea capitis" ~ NA,
      Diagnosa_16 == "L240 - Irritant contact dermatitis due to detergents" ~ NA,
      Diagnosa_16 == "L270 - Generalized skin eruption due to drugs and medicaments" ~ NA,
      Diagnosa_16 == "L271 - Localized skin eruption due to drugs and medicaments" ~ NA,
      Diagnosa_16 == "L272 - Dermatitis due to ingested food" ~ NA,
      Diagnosa_16 == "L300 - Nummular dermatitis" ~ NA,
      Diagnosa_16 == "L309 - Dermatitis, unspecified" ~ NA,
      Diagnosa_16 == "L42 - Pityriasis rosea" ~ NA,
      Diagnosa_16 == "L500 - Allergic urticaria" ~ NA,
      Diagnosa_16 == "L700 - Acne vulgaris" ~ NA,
      Diagnosa_16 == "L710 - Perioral dermatitis" ~ NA,
      Diagnosa_16 == "L732 - Hidradenitis suppurativa" ~ NA,
      Diagnosa_16 == "L743 - Miliaria, unspecified" ~ NA,
      Diagnosa_16 == "N12 - Tubulo-interstitial nephritis, not specified as acute or chronic" ~ NA,
      Diagnosa_16 == "N390 - Urinary tract infection, site not specified" ~ NA,
      Diagnosa_16 == "N47 - Redundant prepuce, phimosis and paraphimosis" ~ NA,
      Diagnosa_16 == "N508 - Other specified disorders of male genital organs" ~ NA,
      Diagnosa_16 == "N61 - Inflammatory disorders of breast" ~ NA,
      Diagnosa_16 == "N645 - Other signs and symptoms in breast" ~ NA,
      Diagnosa_16 == "N700 - Acute salpingitis and oophoritis" ~ NA,
      Diagnosa_16 == "N760 - Acute vaginitis" ~ NA,
      Diagnosa_16 == "N762 - Acute vulvitis" ~ NA,
      Diagnosa_16 == "N768 - Other specified inflammation of vagina and vulva" ~ NA,
      Diagnosa_16 == "O000 - Abdominal pregnancy" ~ NA,
      Diagnosa_16 == "O030 - Spontaneous abortion, incomplete, complicated by genital tract and pelvic infection" ~ NA,
      Diagnosa_16 == "O25 - Malnutrition in pregnancy" ~ NA,
      Diagnosa_16 == "O700 - First degree perineal laceration during delivery" ~ NA,
      Diagnosa_16 == "O701 - Second degree perineal laceration during delivery" ~ NA,
      Diagnosa_16 == "O702 - Third degree perineal laceration during delivery" ~ NA,
      Diagnosa_16 == "O703 - Fourth degree perineal laceration during delivery" ~ NA,
      Diagnosa_16 == "O800 - Spontaneous vertex delivery" ~ NA,
      Diagnosa_16 == "O801 - Spontaneous breech delivery" ~ NA,
      Diagnosa_16 == "O808 - Other single spontaneous delivery" ~ NA,
      Diagnosa_16 == "O809 - Single spontaneous delivery, unspecified" ~ NA,
      Diagnosa_16 == "O839 - Assisted single delivery, unspecified" ~ NA,
      Diagnosa_16 == "O921 - Cracked nipple associated with childbirth" ~ NA,
      Diagnosa_16 == "R040 - Epistaxis" ~ NA,
      Diagnosa_16 == "R560 - Febrile convulsions" ~ NA,
      Diagnosa_16 == "R51 - Headache" ~ NA,
      Diagnosa_16 == "T151 - Foreign body in conjunctival sac" ~ NA,
      Diagnosa_16 == "T171 - Foreign body in nostril" ~ NA,
      Diagnosa_16 == "T301 - Burn of first degree, body region unspecified" ~ NA,
      Diagnosa_16 == "T302 - Burn of second degree, body region unspecified" ~ NA,
      Diagnosa_16 == "T753 - Motion sickness" ~ NA,
      Diagnosa_16 == "T782 - Anaphylactic shock, unspecified" ~ NA,
      Diagnosa_16 == "T812 - Accidental puncture and laceration during a procedure, not elsewhere classified" ~ NA,
      Diagnosa_16 == "Z719 - Counselling, unspecified" ~ NA,
      Diagnosa_16 == "Z370 - Single live birth" ~ NA,
      TRUE ~ Diagnosa_16)) %>%
  select(-Diagnosa_16)
...
data <- data %>%
  mutate(Diagflag = paste0(as.character(Diagnosa_1),";",
                           ...
                           as.character(Diagnosa_015),";",
                           as.character(Diagnosa_016)))

data$Flag <- with(data,
                  ifelse(Diagflag == "NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA",
                         "Non Spesialistik", "Spesialistik"))
data$Diagflag <- as.character(trimws(gsub("NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA;NA|;NA;NA;NA;NA;NA|;NA;NA;NA;NA|;NA;NA;NA|;NA;NA|;NA|NA;|NA","",data$Diagflag)),"both")

data <- data %>%
  select(-Diagnosa_1,-Diagnosa_2,-Diagnosa_3,-Diagnosa_4,-Diagnosa_5,
         -Diagnosa_6,-Diagnosa_7,-Diagnosa_8,-Diagnosa_9,-Diagnosa_010,
         -Diagnosa_011,-Diagnosa_012,-Diagnosa_013,
         -Diagnosa_014,-Diagnosa_015,-Diagnosa_016)
...
data <- data %>%
  mutate(absensi = as.integer(difftime(Tglstjkeu,Tglplgsjp,units = 'days')))
...
write.csv(df_data, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_test.csv",
          na="", row.names = FALSE)
