...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmdati2Layan,Kdppklayan,Nmtkp,
          Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
  ...
  filter(str_detect(Diagnosa,c("A010|A90|A91|A099|N390|R11")))
...
data <- data %>%
  mutate(Keterangan = case_when(
    str_detect(Diagnosa,c("A010")) ~ "Typhoid fever",
    str_detect(Diagnosa,c("A90")) ~ "Dengue fever [classical dengue]",
    str_detect(Diagnosa,c("A91")) ~ "Dengue haemorrhagic fever",
    str_detect(Diagnosa,c("A099")) ~ "Gastroenteritis and colitis of unspecified origin",
    str_detect(Diagnosa,c("N390")) ~ "Urinary tract infection, site not specified",
    str_detect(Diagnosa,c("R11")) ~ "Nausea and vomiting",
    TRUE ~ "Salah"))
...
data <- data %>%select(Nokapst,Nosjp,Umur,Jkpst,Nmdati2Layan,Nmppklayan,Nmtkp,
                       Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
                       Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,
                       Namadpjp,Nmjnspulang,biayars,Biayaverifikasi,Keterangan) %>%
  select(-Diagnosa)
write.csv(data, "D://data gresik//MTF KC GRESIK//gresik_2020//tifoid.csv",
          na="", row.names = FALSE)

write.xlsx(data, file = "Odontektomi_ranap.xlsx")


data21 <- read_csv("Sheet 1_Full Data gresik 2021.csv") %>%
  select (Nokapst,Kelashak,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,
          Politujsjp,Kelasrsmenkes,Klsrawat,Nmdati2Layan,
          Tglpelayanan,Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,
          Nminacbgs,kdsd,kdsi,kdsp,kdsr,Kddiagprimer,
          Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,Tarifgroup,biayars,Biayaverifikasi) %>% 
  mutate (selisih = Biayaverifikasi - Tarifgroup) %>%
  filter(str_detect(Procedure,c("2319")))
...
data <- rbind(data21,data22,data23,data24)
rm(data21,data22,data23,data24)
...
data <- data %>% select(Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Nmppklayan,
                        Tgldtgsjp,Tglplgsjp,Namadpjp,Kdinacbgs,
                        Kddiagprimer,Nmdiagprimer,Diagsekunder,
                        Procedure,biayars,Biayaverifikasi,selisih) %>%
  subset(Nmtkp == "RJTL") %>%
  dplyr::filter(selisih != 0) 
