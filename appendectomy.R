...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Kdppklayan,Nmdati2Layan,
          Nmtkp,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,biayars,Biayaverifikasi) %>%
  filter(str_detect(Procedure, c("ysterectomy")))
...
data <- data %>% subset(Tglplgsjp >= "2016-01-01")

data <- data %>%
  group_by (Nokapst) %>%
  dplyr :: mutate (histerektomi =  sequence(n()))

anomali <- data %>%
  subset(histerektomi > 1)

anomali <- data %>%
  subset(Nokapst %in% c("19649842","93015342","34194182","2701736",
                        "4076390","329634603","69973589"))

anomali <- anomali %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmdati2Layan,Nmppklayan,
          Nmtkp,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
          Diagsekunder,Procedure,biayars,Biayaverifikasi,histerektomi)
anomali <- anomali[order(anomali$Nokapst, anomali$Tglplgsjp),]
anomali <- anomali %>%
  group_by (Nokapst) %>%
  dplyr :: mutate (histerektomi =  sequence(n()))
...
#========================================================================
write.xlsx(anomali, file = "histerektomi.xlsx")
#========================================================================
