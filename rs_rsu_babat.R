...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  ...
  subset(Kdppklayan %in% c("0204R012","0204R019")) %>%
  subset(Nmjnspulang == "Rujuk") %>%
  subset(Nmtkp == "RITL") %>%
  subset(Biayaverifikasi >= biayars)
...
data23 <- data23 %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Nmdati2Layan,Nmppklayan,
          ...
          Nmjnspulang,LOS,biayars,Biayaverifikasi)
...
data23 <- data23[order(data23$Tglplgsjp),]
...
#========================================================================
write.xlsx(data23, file = "RS Muh Babat Rujuk.xlsx")
#========================================================================
