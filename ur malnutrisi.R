...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Kdppklayan,Kelasrsmenkes,
          Nmdati2Layan,Politujsjp,Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
  ...
  filter(str_detect(Diagnosa,c("Malnutrition|malnutrition"))) %>%
  select(-Diagnosa)
...
data23 <- data %>% subset(Tglpelayanan >= "2023-01-01")

data23 <- data23 %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Nmppklayan,Kelasrsmenkes,
          Nmdati2Layan,Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
          Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
          Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi)

data23$Diagprimer <- paste0(as.character(data23$Kddiagprimer)," - ",
                        as.character(data23$Nmdiagprimer))
data23$LOS <- as.integer(data23$Tglplgsjp) - as.integer(data23$Tgldtgsjp) +1
data1 <- data %>%
  subset(Tglpelayanan >= "2016-01-01") %>%
  subset(LOS < 3) %>%
  mutate(Keterangan = "LOS < 3 hari")
data2 <- data %>%
  subset(Tglpelayanan >= "2016-01-01") %>%
  subset(LOS  >= 3 & LOS <= 5) %>%
  mutate(Keterangan = "LOS 3-5 hari")
data3 <- data %>%
  subset(Tglpelayanan >= "2016-01-01") %>%
  subset(LOS > 5) %>%
  mutate(Keterangan = "LOS > 5 hari")

data <- rbind(data1,data2,data3)
rm(data1,data2,data3)

data23 <- data23[order(data23$Tglplgsjp,data23$Nosjp),]

data23 <- data23 %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Nmppklayan,Kelasrsmenkes,
          Nmdati2Layan,Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
          LOS,Kdinacbgs,Nminacbgs,Diagprimer,Diagsekunder,
          Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi)

data$Flag <- with(data,
                  ifelse(str_detect(Diagprimer, c("Pneumonia|pneumonia")),
...
data$Sevel <- substr(data$Kdinacbgs, 8, 10)

write.csv(data, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_pneumonia.csv",
          na="", row.names = FALSE)

#========================================================================
write.xlsx(data23, file = "malnutrisi_auditmedis.xlsx")
#========================================================================
