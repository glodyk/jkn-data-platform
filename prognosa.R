...
fktp <- read.xlsx(xlsxFile = "Bahan Prognosa Bipelkes.xlsx",
                      sheet = "Sheet1",
                      colNames = TRUE,
                      rowNames = FALSE,
                      detectDates = TRUE,
                      ...
                      sep.names = " ") %>%
  select(BUBAN,AKUN,SE33,`KASUS DISETUJUI`,`BIAYA DISETUJUI`,
         `JENIS PELAYANAN`) %>%
  rename(jenis = `JENIS PELAYANAN`,
         kasus = `KASUS DISETUJUI`,
         biaya = `BIAYA DISETUJUI`)

fktp$BUBAN <- as.character(trimws(gsub("-","",substr(fktp$BUBAN,3,6)), "both"))

fktl <- read.xlsx(xlsxFile = "Bahan Prognosa Bipelkes.xlsx",
                  sheet = "Sheet2",
                  colNames = TRUE,
                  rowNames = FALSE,
                  detectDates = TRUE,
                  ...
                  fillMergedCells = TRUE,
                  sep.names = " ") %>%
  select(BUBAN,`AKUN (RI/RJ)`,`JENIS PELAYANAN (untuk KEU)`,
         `Kode Jenis Pelayanan RI/ RJ/ Alkes/ Obat/ Ambulans`,
         `KASUS LAYAK`,`BIAYA LAYAK`) %>%
  rename(AKUN = `AKUN (RI/RJ)`,
         SE33 = `JENIS PELAYANAN (untuk KEU)`,
         jenis = `Kode Jenis Pelayanan RI/ RJ/ Alkes/ Obat/ Ambulans`,
         kasus = `KASUS LAYAK`,
         biaya = `BIAYA LAYAK`)

fktl$TAHUN <- as.character(trimws(gsub("-","",substr(fktl$BUBAN,3,4)), "both"))
fktl$BULAN <- as.character(trimws(gsub("-","",substr(fktl$BUBAN,1,2)), "both"))
fktl$BUBAN0 <- paste0(fktl$TAHUN,fktl$BULAN)

fktl <- fktl %>%
  select(-BUBAN,-TAHUN,-BULAN) %>%
  rename(BUBAN = BUBAN0)

faskes <- rbind(fktl,fktp)
rm(fktl,fktp)

faskes$BUBAN <- as.character(faskes$BUBAN)
faskes$kasus <- as.numeric(faskes$kasus, rm.na = TRUE)

faskes <- faskes %>%
  group_by(BUBAN,AKUN,SE33,jenis) %>%
  dplyr::summarise(kasus = sum(kasus),
                   biaya = sum(biaya))
write.xlsx(faskes, file = "Prognosa2024.xlsx")

capmar24 <- read.xlsx(xlsxFile = "Bahan Prognosa Bipelkes.xlsx",
                      sheet = "Sheet1",
                      colNames = TRUE,
                      rowNames = FALSE,
                      detectDates = TRUE,
...
capmar24$`Jumlah Kontak` <- gsub('\\.', '', as.character(capmar24$`Jumlah Kontak`))
capmar24$`Peserta Terdaftar` <- gsub('\\.', '', as.character(capmar24$`Peserta Terdaftar`))
capmar24$`Angka Kontak` <- gsub('\\,', '.', as.character(capmar24$`Angka Kontak`))
capmar24$`Kasus Non Spesialistik` <- gsub('\\.', '', as.character(capmar24$`Kasus Non Spesialistik`))
capmar24$`Jumlah Rujukan` <- gsub('\\.', '', as.character(capmar24$`Jumlah Rujukan`))
capmar24$RRNS <- gsub('\\,', '.', as.character(capmar24$RRNS))
capmar24$`Prolanis DM Terkendali` <- gsub('\\.', '', as.character(capmar24$`Prolanis DM Terkendali`))
capmar24$`Peserta Diangnosa DM` <- gsub('\\.', '', as.character(capmar24$`Peserta Diangnosa DM`))
capmar24$`RPPT DM` <- gsub('\\,', '.', as.character(capmar24$`RPPT DM`))
capmar24$`Prolanis HT Terkendali` <- gsub('\\.', '', as.character(capmar24$`Prolanis HT Terkendali`))
capmar24$`Peserta Diangnosa HT` <- gsub('\\.', '', as.character(capmar24$`Peserta Diangnosa HT`))
capmar24$`RPPT HT` <- gsub('\\,', '.', as.character(capmar24$`RPPT HT`))
capmar24$`RPPT` <- gsub('\\,', '.', as.character(capmar24$`RPPT`))

capmar24[]=lapply(capmar24,type.convert,as.is=TRUE)
capmar24$`Bulan Kapitasi` <- "01/03/2024"
capmar24$`Bulan Kapitasi` <- as.Date(capmar24$`Bulan Kapitasi`, format = "%d/%m/%Y")

capmar24 <- capmar24 %>%
  select(`Bulan Kapitasi`,Dati2,`Kode PPK`,Faskes,`Peserta Terdaftar`,
         `Jumlah Kontak`,`Angka Kontak`,`Kasus Non Spesialistik`,
         `Jumlah Rujukan`,RRNS,`Peserta Diangnosa DM`,`Prolanis DM Terkendali`,
         `RPPT DM`,`Peserta Diangnosa HT`,`Prolanis HT Terkendali`,
         `RPPT HT`,RPPT)

kapmar24 <- read.xlsx(xlsxFile = "Kapitasi All_mar24.xlsx",
                      sheet = "Sheet 1",
                      colNames = TRUE,
                      rowNames = FALSE,
                      detectDates = TRUE,
                      ...
                      fillMergedCells = TRUE,
                      sep.names = " ") %>%
  subset(`Jenis Kapitasi` == "Regular") %>%
  subset(`KBK Konsekuensi` %in% c("Ya")) %>%
  select(-Jarkomdat,-`Kapitasi Khusus`,-`KBK Konsekuensi`,-`Koefisien RAC`,
         -`Jenis Kapitasi`,-`Usia 0 5 L`,-`Usia 6 20 L`,-`Usia 21 60 L`,
         -`Usia 60Plus L`,-`Usia 0 5 P`,-`Usia 6 20 P`,-`Usia 21 60 P`,
         -`Usia 60Plus P`,-`Besaran Tarif Kapitasi Awal`,-`By Usia 0 5 L`,
         -`By Usia 0 5 P`,-`By Usia 6 20 L`,-`By Usia 6 20 P`,
         -`By Usia 21 60 L`,-`By Usia 21 60 P`,-`By Usia 60Plus L`,
         -`By Usia 60Plus P`,-`Biaya Kapitasi Awal`,
         -`Besaran Tarif Kapitasi RAC`,-`Biaya Kapitasi RAC`,
         -`Besaran Tarif Kapitasi RAC KBK`,-`Biaya Kapitasi RAC KBK`,
         -`Biaya Kapitasi Akhir`,-`Besaran Tarif Kapitasi Akhir`,
         -`Capaian KBK`,-`Biaya Kapitasi Bayar`)

library(splitstackshape)
kapmar24 <- concat.split(kapmar24, "Kode dan Nama FKTP", " - ")
kapmar24 <- kapmar24 %>%
  rename(`Kode PPK` = "Kode dan Nama FKTP_1",
         Faskes = "Kode dan Nama FKTP_2") %>%
  select(-`Kode dan Nama FKTP`,-Faskes)

lampmar24 <- read.xlsx(xlsxFile = "Lampiran Kapitasi_mar24.xlsx",
                       sheet = "Sheet 1",
                       colNames = TRUE,
                       rowNames = FALSE,
                       detectDates = TRUE,
  ...
  subset(`Jenis Kapitasi` == "Regular") %>%
  subset(`KBK Konsekuensi` %in% c("Ya")) %>%
  select(-No,-`KBK Konsekuensi`,-`Jenis Kapitasi`,-`Usia 0 5 L`,-`Usia 6 20 L`,
         -`Usia 21 60 L`,-`Usia 60Plus L`,-`Usia 0 5 P`,-`Usia 6 20 P`,
         -`Usia 21 60 P`,-`Usia 60Plus P`,-`Besaran Tarif Kapitasi Awal`,
         -`Besaran Tarif Kapitasi Akhir`,
         -`Nama FKTP`,-`Jenis Fktp`)

bymar24 <- left_join(kapmar24,lampmar24, by = c("Kode PPK"="Kode FKTP"))
mar24 <- left_join(bymar24,capmar24, by = c("Kode PPK"="Kode PPK"))

mar24 <- mar24 %>%
  select(`Bulan Kapitasi`,Dati2,`Jenis Fktp`,Faskes,`Peserta Terdaftar`,
         `Total Peserta Terdaftar`,
         `Jumlah Kontak`,`Angka Kontak`,`Kasus Non Spesialistik`,
         `Jumlah Rujukan`,RRNS,`Peserta Diangnosa DM`,`Prolanis DM Terkendali`,
         `RPPT DM`,`Peserta Diangnosa HT`,`Prolanis HT Terkendali`,
         `RPPT HT`,RPPT,`Capaian KBK`,`Jumlah Dokter`,`Jumlah Dokter Gigi`,`Rasio Dokter`,
         `Biaya Kapitasi Bayar`)
rm(capmar24,kapmar24,lampmar24,bymar24)
write.xlsx(mar24, file = "Koreksi_Kap_Mar24.xlsx")
#=====================================================================================
capapr24 <- read.xlsx(xlsxFile = "Tabel Capaian_kbk apr 24.xlsx",
                      sheet = "Sheet 1",
                      colNames = TRUE,
                      rowNames = FALSE,
                      detectDates = TRUE,
                      ...
                      fillMergedCells = TRUE,
                      sep.names = " ")
capapr24$`Jumlah Kontak` <- gsub('\\.', '', as.character(capapr24$`Jumlah Kontak`))
capapr24$`Peserta Terdaftar` <- gsub('\\.', '', as.character(capapr24$`Peserta Terdaftar`))
capapr24$`Angka Kontak` <- gsub('\\,', '.', as.character(capapr24$`Angka Kontak`))
capapr24$`Kasus Non Spesialistik` <- gsub('\\.', '', as.character(capapr24$`Kasus Non Spesialistik`))
capapr24$`Jumlah Rujukan` <- gsub('\\.', '', as.character(capapr24$`Jumlah Rujukan`))
capapr24$RRNS <- gsub('\\,', '.', as.character(capapr24$RRNS))
capapr24$`Prolanis DM Terkendali` <- gsub('\\.', '', as.character(capapr24$`Prolanis DM Terkendali`))
capapr24$`Peserta Diangnosa DM` <- gsub('\\.', '', as.character(capapr24$`Peserta Diangnosa DM`))
capapr24$`RPPT DM` <- gsub('\\,', '.', as.character(capapr24$`RPPT DM`))
capapr24$`Prolanis HT Terkendali` <- gsub('\\.', '', as.character(capapr24$`Prolanis HT Terkendali`))
capapr24$`Peserta Diangnosa HT` <- gsub('\\.', '', as.character(capapr24$`Peserta Diangnosa HT`))
capapr24$`RPPT HT` <- gsub('\\,', '.', as.character(capapr24$`RPPT HT`))
capapr24$`RPPT` <- gsub('\\,', '.', as.character(capapr24$`RPPT`))

capapr24[]=lapply(capapr24,type.convert,as.is=TRUE)
capapr24$`Bulan Kapitasi` <- "01/04/2024"
capapr24$`Bulan Kapitasi` <- as.Date(capapr24$`Bulan Kapitasi`, format = "%d/%m/%Y")
...
kapapr24 <- read.xlsx(xlsxFile = "Kapitasi All_apr24.xlsx",
                      sheet = "Sheet 1",
                      colNames = TRUE,
                      rowNames = FALSE,
                      detectDates = TRUE,
...
library(splitstackshape)
kapapr24 <- concat.split(kapapr24, "Kode dan Nama FKTP", " - ")
kapapr24 <- kapapr24 %>%
  rename(`Kode PPK` = "Kode dan Nama FKTP_1",
         Faskes = "Kode dan Nama FKTP_2") %>%
  select(-`Kode dan Nama FKTP`,-Faskes)

lampapr24 <- read.xlsx(xlsxFile = "Lampiran Kapitasi_apr24.xlsx",
                       sheet = "Sheet 1",
                       colNames = TRUE,
                       rowNames = FALSE,
                       detectDates = TRUE,
...
byapr24 <- left_join(kapapr24,lampapr24, by = c("Kode PPK"="Kode FKTP"))
apr24 <- left_join(byapr24,capapr24, by = c("Kode PPK"="Kode PPK"))
...
apr24 <- apr24 %>%
  select(`Bulan Kapitasi`,Dati2,`Jenis Fktp`,Faskes,`Peserta Terdaftar`,
         ...
         `RPPT HT`,RPPT,`Capaian KBK`,`Jumlah Dokter`,`Jumlah Dokter Gigi`,`Rasio Dokter`,
         `Biaya Kapitasi Bayar`)
rm(capapr24,kapapr24,lampapr24,byapr24)
write.xlsx(apr24, file = "Koreksi_Kap_Apr24.xlsx")
#=====================================================================================
...
capapr24 <- read.xlsx(xlsxFile = "Tabel Capaian_kbk apr 24.xlsx",
                      ...
                      fillMergedCells = TRUE,
                      sep.names = " ")
apr24$`Bulan Kapitasi` <- "01/04/2024"
apr24$`Bulan Kapitasi` <- as.Date(apr24$`Bulan Kapitasi`, format = "%d/%m/%Y")



mar24 <- read.csv2("Kapitasi All_Full Data_data.csv", sep = ",", dec = ",")

mar24 <- read.table("Tabel Capaian_kbk mar 24.csv",header = FALSE,    # Whether to display the header (TRUE) or not (FALSE)
...
bpkp <- xl.read.file("kap_pbijk.xlsx",
                     password = "K4ppbijk",
                     header = TRUE,
                     row.names = NULL,
                     col.names = NULL,
...
datkor <- left_join(data,datkor, by = c("Nosjp","Nmtkp","koreksi",
                                         "Kdinacbgs","Nminacbgs")) %>%
