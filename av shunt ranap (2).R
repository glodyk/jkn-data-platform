...
data19 <- read_csv("Sheet 1_Full Data gresik 2019.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,
          ...
          Biayaverifikasi) %>% 
  subset(Nmtkp == "RITL") %>%
  subset(Procedure == "3927 - Arteriovenostomy for renal dialysis") %>%
  select(-Nmtkp)
...
data <- rbind(data19,data20,data21,data22,data23,data24)
rm(data19,data20,data21,data22,data23,data24)
...
data <- data %>% subset(LOS < 3)
data <- data[order(data$Nmppklayan, data$Tglpelayanan),]
data <- data %>%select(Nokapst,Nosjp,Jkpst,Umur,Nmdati2Layan,
                       Nmppklayan,Tgldtgsjp,LOS,Tglplgsjp,
