...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  ...
  subset(Kdppklayan %in% c("0205R012","1302R002")) %>%
  subset(Nmtkp == "RJTL") %>%
  subset(Politujsjp == "INT") %>%
  mutate(Diagprimer = paste0(as.character(Kddiagprimer)," - ",
                             as.character(Nmdiagprimer))) %>%
  select(-Kddiagprimer,-Nmdiagprimer)
...
#========================================================================
write.xlsx(data, file = "INT_RS Semen Gresik_RS Petrokimia.xlsx")
#========================================================================
