...
data24 <- read_csv("Sheet 1_Full Data gresik jan-mar 2024.csv") %>%
  ...
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Procedure,c("extraction of lens"))) %>%
...
data <- rbind(data24,dataapr24,datamei24,dataagu24)
rm(data24,dataapr24,datamei24,dataagu24)
...
data <- data %>% subset(Tglpelayanan >= "2024-01-01")
data <- data[order(data$Nokapst, data$Tglplgsjp),]

#========================================================================
write.xlsx(data, file = "ECCE ranap RS Intan Medika.xlsx")
#========================================================================
