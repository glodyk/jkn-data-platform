...
datamei24 <- read_csv("Sheet 1_Full Data gresik mei 2024.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmdati2Layan,Kdppklayan,Nmtkp,
          Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,
          Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,
          Namadpjp,Nmjnspulang,biayars,Biayaverifikasi) %>%
...
rm(data16,data17,data18,data19,data20,data21,data22,
   data23,data24,dataapr24,datamei24)

#======================================================================
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>% group_by(Nokapst) %>%
  mutate(Keterangan = paste0("meninggal ke-", sequence(n())))

dobel <- data %>% subset(Keterangan == "meninggal ke-2") %>%
  select(Nokapst)
dobel <- dobel[!duplicated(dobel$Nokapst, fromLast = FALSE),]
dobel <- left_join(dobel, data, by = c("Nokapst"="Nokapst"))

dobel$Tglpelayanan <- as.Date(dobel$Tglpelayanan, format = "%m/%d/%Y")
dobel$Tgldtgsjp <- as.Date(dobel$Tgldtgsjp, format = "%m/%d/%Y")
dobel$Tglplgsjp <- as.Date(dobel$Tglplgsjp, format = "%m/%d/%Y")

dobel <- dobel[order(dobel$Nokapst,dobel$Tglplgsjp),]
dobel <- dobel %>% group_by(Nokapst) %>%
  mutate(Keterangan = paste0("meninggal ke-", sequence(n())))
...
dobel <- dobel %>% 
  mutate(
    Nmppklayan = case_when(
      ...
      TRUE ~ Kdppklayan)) %>%
  select(-Kdppklayan)
dobel <- dobel %>% select(Nokapst,Nosjp,Jkpst,Umur,Nmdati2Layan,
                          Nmppklayan,Nmtkp,Politujsjp,Tglpelayanan,
                          Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
                          Kddiagprimer,Nmdiagprimer,Diagsekunder,
                          Procedure,Namadpjp,Nmjnspulang,biayars,
                          Biayaverifikasi,Keterangan)
#========================================================================
write.xlsx(dobel, file = "dobel meninggal.xlsx")
#========================================================================
nokapst <- data %>% select(Nokapst)
nokapst <- nokapst[!duplicated(nokapst$Nokapst, fromLast = FALSE),]
...
meninggal <- left_join(nokapst, data, by = c("Nokapst"="Nokapst"))

meninggal$Tglpelayanan <- as.Date(meninggal$Tglpelayanan, format = "%m/%d/%Y")
meninggal$Tgldtgsjp <- as.Date(meninggal$Tgldtgsjp, format = "%m/%d/%Y")
meninggal$Tglplgsjp <- as.Date(meninggal$Tglplgsjp, format = "%m/%d/%Y")
...
meninggal <- meninggal[order(meninggal$Nokapst,meninggal$Tglplgsjp),]
meninggal <- meninggal %>%
  group_by(Nokapst) %>%
  mutate(
    Flag = ifelse(Nokapst == lead(Nokapst) &
                     Nmjnspulang=="Meninggal" &
                     lead(Nmjnspulang) != "Meninggal",
                   "Keliru","Benar"))

meninggal <- meninggal %>% group_by(Nokapst) %>%
  mutate(sblmpulang = lag(Nmjnspulang, n=1))

meninggal$Flag <- paste0(as.character(meninggal$sblmpulang)
                                   ," - ",
                                   as.character(meninggal$Nmjnspulang))
meninggal <- meninggal %>%
  subset(Flag != "NA - Meninggal")
meninggal <- meninggal[order(meninggal$Nokapst,meninggal$Tglplgsjp),]
meninggal <- meninggal %>% group_by(Nokapst) %>%
  mutate(sblmpulang = lag(Nmjnspulang, n=1))
...
cek <- meninggal %>%
  subset(Flag == "Keliru")
nokapstcek <- cek %>% select(Nokapst)
nokapstcek <- nokapstcek[!duplicated(nokapstcek$Nokapst, fromLast = FALSE),]
meninggal2 <- left_join(nokapstcek, meninggal, by = c("Nokapst"="Nokapst"))

meninggal2 <- meninggal2[order(meninggal2$Nokapst,meninggal2$Tglplgsjp),]
write.xlsx(meninggal2, file = "meninggal2.xlsx")


meninggal <- meninggal %>%
  group_by(Nokapst) %>%
  mutate(Test = ifelse(is.na(meninggal$Nmjnspulang[row_number() + 1]),
                       "meninggal", "mulai baru"))

         ,
         Test = c(Test[-n()], 0))

library(plyr)
meninggal$Flag2 <- ddply(meninggal, .(Nokapst),
                   summarize, Flag2 = "Meninggal" %in% Nmjnspulang)

meninggal$Flag2 <- with(meninggal,
                        ifelse(Nmjnspulang == "Meninggal" & lead(is.na(sblmpulang)),
                               "meninggal",
                               ifelse(Nmjnspulang != "Meninggal" & lead(Nmjnspulang) != "Meninggal",
                                      "salah"))
datafinal <- datafinal %>%
  group_by(Nokapst) %>% 
...
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>% group_by(Nokapst) %>%
  mutate(Keterangan = paste0("hemodialisa ke-", sequence(n())))
...
data <- data %>%
  select(Nokapst,Nosjp,Jkpst,Umur,Nmdati2Layan,Nmppklayan,Nmtkp,
         Politujsjp,Tglpelayanan,Tgldtgsjp,Tglplgsjp,tgl_before,jeda,
         Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
         Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi,
         Keterangan)

write.csv(data, "D:/data gresik/MTF KC GRESIK/gresik_2020/db hemodialisa.csv",
          na="", row.names = FALSE)
#========================================================================
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data$Flag <- with(data,
                  ifelse(data$Nokapst == lag(data$Nokapst),
                         "berulang", "baru"))
data$Flag2 <- with(data,
                   ifelse(is.na(data$Flag), "baru",
                          data$Flag))
data$Flag3 <- with(data,
                   ifelse(data$Nokapst == lead(data$Nokapst),
                          "berulang", "baru"))
data$ket <- paste0(as.character(data$Flag2),"-",
                   as.character(data$Flag3))
data <- data %>%
  filter(str_detect(ket,c("berulang")))

data <- data %>% group_by(Nokapst) %>%
  mutate(interval = as.numeric(Tgldtgsjp-lag(Tglplgsjp)))
data$interval2 <- with(data,
                       ifelse(is.na(data$interval), lead(data$interval),
                              data$interval))
data$ket2 <- with(data,
                       ifelse(data$interval2 < 3, "salah","benar"))
data$ket3 <- with(data,
                   ifelse(data$Nokapst == lead(data$Nokapst),
                          lead(data$ket2), data$ket2))
data$ket4 <- paste0(as.character(data$ket2),"-",
                   as.character(data$ket3))
data <- data %>%
  filter(str_detect(ket4,c("salah")))

data <- data %>%
  select(Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Nmppklayan,Tgldtgsjp,
         Tglplgsjp,interval,Kdinacbgs,Kddiagprimer,Nmdiagprimer,
...
#========================================================================
write.xlsx(data, file = "hemodialisa.xlsx")
#========================================================================
