...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Kdppklayan,Kelasrsmenkes,
          Nmdati2Layan,Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
          Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
          Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi) %>%
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Procedure,c("9671|9672"))) %>%
  subset(Kdppklayan == "0204R010")
...
data <- data[order(data$Kdppklayan, data$Tglpelayanan),]
...
data$Keterangan <- with(data,
                        ifelse(str_detect(Procedure,
                                          c("9671")),"kurang dari 96 jam",
                                "lebih dari 96 jam"))
...
data1$meninggal <- with(data1,
                        ifelse(str_detect(Nmjnspulang,
                                          c("Meninggal")),"Meninggal",
                               "Tidak Meninggal"))

data1 <- data1 %>%
  subset(meninggal == "Meninggal")

data1 <- data1 %>%
  subset(Keterangan == "kurang dari 96 jam")

data1$potensi <- with(data1,
                      ifelse(LOS <= 3 & meninggal == "Meninggal",
                             "No Medical Value",
                             "Cek kembali jam ventilator"))

data1 <- data1 %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Nmppklayan,
          Kelasrsmenkes,Nmdati2Layan,Klsrawat,Tglpelayanan,Tgldtgsjp,
          Tglplgsjp,LOS,Kdinacbgs,Nminacbgs,Diagprimer,Diagsekunder,
          Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi,Keterangan)

data1 <- data1[order(data1$Tglplgsjp,data1$Nosjp),]

#========================================================================
write.xlsx(data1, file = "ventilator_rsml.xlsx")
#========================================================================
...
df_jan19 <- read_csv("Sheet_1_Full_Data_data gresik jan 19.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Nmtkp,Nmppklayan,Tgldtgsjp,Tglplgsjp,
          Kdinacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,
          Namadpjp,Nmjnspulang,biayars,Biayaverifikasi) %>%
  filter(str_detect(Procedure,c("9672")))
...
data <- data %>%
  select (Nokapst,Nosjp,Jkpst,Nmtkp,Nmppklayan,Tgldtgsjp,Tglplgsjp,
          LOS,Kdinacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
          Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi)

data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>%
  group_by(Nokapst) %>%
  mutate(kunjungan = sequence(n()))
#========================================================================
...
write.xlsx(data, file = "9672.xlsx")


data <- data %>% subset(interval < 3)
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data <- data %>% group_by(Nokapst) %>%
  mutate(interval = as.numeric(Tgldtgsjp-lag(Tglplgsjp)))
data$interval <- as.numeric(data$interval) %>%
