...
synovectomy <- data  %>%
  filter(str_detect(Procedure,"Synovectomy|synovectomy")) %>%
  subset(Tglstjkeu >= "2024-01-01") %>%
...
synovectomy <- synovectomy[order(synovectomy$Nokapst,synovectomy$Tglplgsjp),]
write.xlsx(synovectomy, file = "synovectomy.xlsx")


rujuk <- data  %>%
  subset(Tglstjkeu >= "2024-08-01") %>%
  subset(Nmppklayan %in% c("RS Petrokimia Gresik","RS Intan Medika","RSU Muhammadiyah Babat")) %>%
  subset(Nmjnspulang == "Rujuk") %>%
  subset(Nmtkp == "RITL") %>%
  mutate(LOS = as.integer(difftime(Tglplgsjp,Tgldtgsjp,units = 'days'))) %>%
  subset(LOS <= 3) %>%
  select(Nokapst,Nosjp,Umur,Jkpst,Nmppkperujuk,
         Nmdati2Layan,Nmppklayan,Nmtkp,Tglpelayanan,
         Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,
...
rujuk <- rujuk[order(rujuk$Nokapst,rujuk$Tglplgsjp),]
write.xlsx(rujuk, file = "Rujuk Antar RS LOS kurang dari sama dengan 3 hari di 3 RS Lokus.xlsx")
...
ganglion <- data %>%
  subset(Tglstjkeu >= "2024-01-01") %>%
  subset(Nmtkp == "RITL") %>%
  filter(str_detect(Diagnosa,"E105|E115")) %>%
  filter(str_detect(Procedure,"8341|8342|8343|8344|8345|8349")) %>%
  select(Nokapst,Nosjp,Umur,Jkpst,Nmppkperujuk,
         Nmdati2Layan,Nmppklayan,Nmtkp,Tglpelayanan,
         Tgldtgsjp,Tglplgsjp,Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,
         Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,Nmjnspulang,biayars,
         Biayaverifikasi)
write.xlsx(ganglion, file = "dx E10.5 atau E11.5 kode prosedur 83.45.xlsx")
ganglion <- ganglion[order(ganglion$Nokapst,ganglion$Tglplgsjp),]

softtissue <- data %>%
  subset(Nmtkp == "RITL")%>%
  subset(Nmppklayan == "RS Petrokimia Gresik") %>%
  subset(Tglpelayanan >= "2024-01-01" & Tglpelayanan <= "2024-11-01") %>%
  filter(str_detect(Procedure,"811|863|864|1839|2051|8339|8349|8604|8609|8622|8737")) %>%
  select(-Nmtkp)

softtissue <- softtissue %>%
  select(Nmppklayan,Nokapst,Umur,Jkpst,,Nosjp,Tgldtgsjp,Tglplgsjp,
         Tglstjkeu,Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
         Procedure,Nmjnspulang,Biayaverifikasi,Namadpjp)
softtissue <- softtissue[order(softtissue$Nokapst,softtissue$Tglplgsjp),]

write.xlsx(softtissue, file = "Bahan Audit Medis STT Petro.xlsx")
#========================================================================
