...
data22 <- read_csv("Sheet 1_Full Data gresik 2022.csv") %>%
  ...
  subset(Nmtkp == "RITL") %>%
  mutate(Sevel = as.character(trimws(substr(Kdinacbgs,8,10)), "both")) %>%
  subset(Sevel %in% c("II","III"))
...
data$Keterangan <- "LOS 1-2 hari & Sevel II-III"

data <- data[order(data$Nokapst, data$Tglplgsjp),]
#========================================================================
write.xlsx(data, file = "LOS 1-2 hari & Sevel II-III.xlsx")
#========================================================================
