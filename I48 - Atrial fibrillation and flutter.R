...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  select (Nokapst,Nosjp,Umur,Jkpst,Nmtkp,Nmdati2Layan,Kdppklayan,
          Politujsjp,Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,
          Kdinacbgs,Nminacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
          Procedure,Namadpjp,Nmjnspulang,biayars,Biayaverifikasi,Kdcmg) %>%
  subset(Nmtkp == "RITL") %>% 
  subset(Kdcmg == "I") %>%
  filter(str_detect(Diagsekunder,c("I48 - Atrial fibrillation and flutter|I500|
                    I110"))) %>%
  select(-Kdcmg)
...
data$Keterangan <- with(data,
                        ifelse(str_detect(Diagsekunder,"I48 - Atrial fibrillation and flutter"),
                               "AF and flutter tidak perlu dikoding pada DS",
                               ifelse(str_detect(Diagsekunder,"I500"),
                                      "CHF tidak perlu dikoding pada DS (bila tidak ada terapi khusus)",
                                      ifelse(str_detect(Diagsekunder,"I110"),
                                             "HHD with CHF tidak perlu dikoding pada DS (bila tidak ada terapi khusus)",
                                             "Salah"))))

data <- data[order(data$Nokapst, data$Tglpelayanan),]
data <- data %>%
...
data22 <- data22[order(data22$Nokapst, data22$Tglpelayanan),]
write.xlsx(data, file = "Cek CMG I.xlsx")
