...
data16 <- read_csv("Sheet 1_Full Data gresik 2016.csv") %>%
  select (Nokapst,Umur,Jkpst,Nmdati2Layan,Nmppkperujuk,Kdppklayan,Nosjp,
          ...
          Namadpjp,Nmjnspulang,Tarifgroup,biayars,Biayaverifikasi) %>%
  subset(Nmtkp == "RJTL") %>%
  ...
  filter(str_detect(Diagnosa,"disorder of speech")) %>%
  select(-Diagnosa)
...
data <- data %>%
  mutate(Keterangan = case_when(
    str_detect(Kdinacbgs,c("Z-3-12-0")) ~ "Speech therapy",
    str_detect(Procedure,c("9374|9375|9383|9433")) ~ "Speech therapy",
    str_detect(Procedure,c("9402|9408")) ~ "Psychologic test",
    str_detect(Procedure,c("9334|9335|9339")) ~ "Physical therapy",
    str_detect(Kdinacbgs,c("M-3-16-0")) ~ "Physical therapy",
    TRUE ~ "lain-lain"))

data <- data[order(data$Nokapst, data$Tglplgsjp),]
data <- data %>% group_by (Nokapst,Keterangan,Tglpelayanan) %>% 
  dplyr :: mutate (kunjungan =  sequence(n()))

data$kel_umur <- with(data,
                      ifelse(Umur >= 0 & Umur < 1, "< 1 Tahun",
                             ifelse(Umur >= 1 & Umur < 5, "1-4 Tahun",
                                    ifelse(Umur >= 5 & Umur < 8, "5-7 Tahun",
                                           ifelse(Umur >= 8 & Umur < 12, "8-11 Tahun",
...
#========================================================================
write.xlsx(data, file = "RS Grhu disorder of speech.xlsx")
#========================================================================
