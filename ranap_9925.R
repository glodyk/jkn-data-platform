...
df_jan19 <- read_csv("Sheet_1_Full_Data_data gresik jan 19.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Nmtkp,Nmppklayan,Tgldtgsjp,Tglplgsjp,
          ...
          Procedure,Namadpjp,Kdjnspulang,Biayaverifikasi) %>%
  subset(Nmtkp == "RITL" & Kdjnspulang == "1") %>%
  subset(Kddiagprimer == "Z511") %>%
  subset(Procedure == "9925 - Injection or infusion of cancer chemotherapeutic substance")
...
data <- data %>%
  subset(interval < 30)
data <- data %>%
  select(-Kdjnspulang,-Nmkronis,-interval,-int_next,-Flag,
         -Flag2,-ket)
data <- data %>%
...
write.xlsx(data, file = "ranap kemoterapi.xlsx")
#========================================================================
