...
data22 <- read_csv("Sheet 1_Full Data gresik 2022.csv") %>%
  ...
  filter(str_detect(Kdinacbgs, c("I-1-40|I-1-15"))) %>%
  filter(!str_detect(Namadpjp, c("Hari Yudha|HARI YUDHA")))
...
data <- rbind(data22,data23,data24,dataapr24,datamei24)
rm(data22,data23,data24,dataapr24,datamei24)
...
data <- data %>% subset(Tglpelayanan >= "2022-12-01")
...
write.xlsx(data, file = "Cathlab tidak tercantum BAPETEN.xlsx")
#==================================================================
