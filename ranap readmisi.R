...
df_des20 <- read_csv("Sheet_1_Full_Data_data gresik des 20.csv") %>%
  filter(str_detect(Procedure,c("endoscopy")))

#========================================================================
df_jan19 <- read_csv("Sheet_1_Full_Data_data gresik jan 19.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Nmtkp,Nmppklayan,Tgldtgsjp,Tglplgsjp,
          Kdinacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
          Procedure,Namadpjp,Kdjnspulang,biayars,Biayaverifikasi,
          Nmkronis) %>%
  subset(Nmtkp == "RITL" & Kdjnspulang == "1") %>%
  filter(!str_detect(Kddiagprimer,c("D66"))) %>%
  filter(!str_detect(Kdinacbgs,c("O|C-4-13"))) %>%
  filter(str_detect(Nmkronis,c("KRONIS")))
...
data$Flag2 <- with(data,
                  ifelse(is.na(data$Flag), "nubie",
                         data$Flag))
data <- data %>% group_by(Nokapst) %>%
  mutate(int_next = as.numeric(lead(Tgldtgsjp)-Tglplgsjp))
data$int_next <- as.numeric(data$int_next) %>% 
  replace(is.na(.), 0)
data$ket <- paste0(as.character(data$interval),"-",
                   as.character(data$int_next),"-",
                   data$Flag2)
data <- data %>%
  subset(data$ket != "0-0-nubie")

data <- data %>%
  select(-interval,-int_next,-Flag,-Flag2,-ket)
data <- data %>%
  select(-Kdjnspulang,-Nmkronis)
#========================================================================
data <- data[order(data$Nokapst,data$Tglplgsjp),]
data$Flagppk <- with(data,
                      ifelse(data$Nokapst == lag(data$Nokapst) &
                               data$Nmppklayan == lag(data$Nmppklayan),
                             "RS Sama", "RS Beda"))
data <- data %>%
  select(-Flagppk)
#========================================================================
...
data <- data %>%
  subset(interval < 30)
data <- data %>%
  select(-interval,-int_next,-Flag,
         -Flag2,-ket)
data <- data %>%
  select(Nokapst,Nosjp,Jkpst,Nmppklayan,Tgldtgsjp,Tglplgsjp,
         interval,Kdinacbgs,Kddiagprimer,Nmdiagprimer,Diagsekunder,
         Procedure,Namadpjp,biayars,Biayaverifikasi)
write.xlsx(data, file = "ranap readmisi.xlsx")
#========================================================================


data$jeda <- data$interval + data$int_next
data <- data %>% subset(jeda > 1 & jeda <= 60)
...
data <- data %>% group_by(Nokapst) %>%
  mutate(Tglbefore_1 = lag(Tglplgsjp))

data <- data %>% mutate(Tglbefore = ifelse(is.na(Tglbefore_1),
                                             0, Tglbefore_1))
data$Tglbefore <- as.Date(data$Tglbefore, "1970-01-01")
data$interval <- as.integer(data$Tglplgsjp) - as.integer(data$Tglbefore)


data <- data %>% select(-Nmkronis,-Tglbefore_1,-Tglbefore,-interval)
data$Tglbefore <- as.numeric(data$Tglbefore)

data$Tglbefore[is.na(data$Tglbefore)] <- 0


%>%
  replace(is.na(data$Tglbefore),0)

data <- data %>% filter(!is.na(Tglbefore))



%>%
  filter(str_detect(Diagsekunder,c("K92|K29")))
...
write.xlsx(data, file = "anemia.xlsx")
