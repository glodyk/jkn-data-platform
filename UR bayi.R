...
#=============================data nadyah=============================
fktl1 <- read_csv("Sheet 1_Full Data gresik jan-mar 2024.csv") %>%
  select(Biayaverifikasi,Nmdati2Layan,Nmtkp,Nmppklayan,Tglpelayanan,
         Tglstjkeu)
fktl2 <- read_csv("Sheet 1_Full Data gresik apr 2024.csv") %>%
  select(Biayaverifikasi,Nmdati2Layan,Nmtkp,Nmppklayan,Tglpelayanan,
         Tglstjkeu)
fktl <- rbind(fktl1,fktl2)
rm(fktl1,fktl2)

fktl$Tglpelayanan <- as.Date(fktl$Tglpelayanan, format = "%m/%d/%Y")
fktl$Tglstjkeu <- as.Date(fktl$Tglstjkeu, format = "%m/%d/%Y")

rekap <- fktl %>%
  group_by(Nmdati2Layan,Nmppklayan,Nmtkp,Tglpelayanan) %>%
  summarise(Biayaverifikasi)
write.xlsx(rekap, file = "rekapfktl.xlsx")

#=============================data nadyah=============================
data19 <- read_csv("Sheet 1_Full Data gresik 2019.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
...
data <- data %>%
  mutate(Keterangan = case_when(
    str_detect(Kddiagprimer,c("P070")) ~ "BB Lahir < 1.000 gram",
    str_detect(Kddiagprimer,c("P071")) ~ "BB Lahir < 2.500 gram",
    str_detect(Kddiagprimer,c("P210")) ~ "Severe birth asphyxia",
    str_detect(Kddiagprimer,c("P211")) ~ "Mild and moderate birth asphyxia",
    str_detect(Kddiagprimer,c("P219")) ~ "Birth asphyxia, unspecified",
    str_detect(Kddiagprimer,c("P220|P229")) ~ "Respiratory distress",
    str_detect(Kddiagprimer,c("P221")) ~ "Transient takipnea",
    str_detect(Kddiagprimer,c("P24")) ~ "Sindrom aspirasi mekonium",
    str_detect(Kddiagprimer,c("P39")) ~ "Infection perinatal",
    str_detect(Kddiagprimer,c("P59")) ~ "Ikterus neonatus",
    str_detect(Kddiagprimer,c("P70")) ~ "Hipoglikemia",
    str_detect(Kddiagprimer,c("P78")) ~ "Gangguan pencernaan perinatal",
    str_detect(Kddiagprimer,c("P80|P81")) ~ "Gangguan suhu tubuh",
    str_detect(Kddiagprimer,c("P92")) ~ "Poor-feeding neonatal",
    str_detect(Diagsekunder,c("P070")) ~ "BB Lahir < 1.000 gram",
    str_detect(Diagsekunder,c("P071")) ~ "BB Lahir < 2.500 gram",
    str_detect(Diagsekunder,c("P210")) ~ "Severe birth asphyxia",
    str_detect(Diagsekunder,c("P211")) ~ "Mild and moderate birth asphyxia",
    str_detect(Diagsekunder,c("P219")) ~ "Birth asphyxia, unspecified",
    str_detect(Diagsekunder,c("P220|P229")) ~ "Respiratory distress",
    str_detect(Diagsekunder,c("P221")) ~ "Transient takipnea",
    str_detect(Diagsekunder,c("P24")) ~ "Sindrom aspirasi mekonium",
    str_detect(Diagsekunder,c("P39")) ~ "Infection perinatal",
    str_detect(Diagsekunder,c("P59")) ~ "Ikterus neonatus",
    str_detect(Diagsekunder,c("P70")) ~ "Hipoglikemia",
    str_detect(Diagsekunder,c("P78")) ~ "Gangguan pencernaan perinatal",
    str_detect(Diagsekunder,c("P80|P81")) ~ "Gangguan suhu tubuh",
    str_detect(Diagsekunder,c("P92")) ~ "Poor-feeding neonatal",
    str_detect(Kddiagprimer,c("P030|P031|P032|P033|P034|P035|P036")) ~ "Newborn affected by other complications of labor and delivery",
    TRUE ~ "Diagnosa lain-lain"))

#==================================================================
df19 <- read_csv("Sheet 1_Full Data gresik 2019.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
  subset(Nmtkp == "RITL") %>%
  filter(!str_detect(Kdcmg,c("P"))) %>%
...
df20 <- read_csv("Sheet 1_Full Data gresik 2020.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
...
df21 <- read_csv("Sheet 1_Full Data gresik 2021.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
...
df22 <- read_csv("Sheet 1_Full Data gresik 2022.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
...
df23 <- read_csv("Sheet 1_Full Data gresik 2023.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
...
df24 <- read_csv("Sheet 1_Full Data gresik jan-mar 2024.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
...
dfapr24 <- read_csv("Sheet 1_Full Data gresik apr 2024.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
...
dfmei24 <- read_csv("Sheet 1_Full Data gresik mei_jun 2024.csv") %>%
  select (Nokapst,Nosjp,Jkpst,Umur,Nmtkp,Kdppklayan,Nmdati2Layan,
          Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,Nminacbgs,
          Kdcmg,Kddiagprimer,Nmdiagprimer,Diagsekunder,Procedure,Namadpjp,
          Nmjnspulang,biayars,Biayaverifikasi) %>% 
...
df <- rbind(df19,df20,df21,df22,df23,df24,dfapr24,dfmei24)
rm(df19,df20,df21,df22,df23,df24,dfapr24,dfmei24)

df$Keterangan <- with(df,
                      ifelse(str_detect(Procedure,c("Cesarean section|cesarean section")),
                                        "Persalinan SC",
...
df <- df[order(df$Nmppklayan, df$Tglpelayanan),]
df <- df %>% subset(Tglpelayanan >= "2022-01-01")

datax <- rbind(data,df)
rm(data,df)

data <- data[order(data$Tglplgsjp,data$Nosjp),]
data <- data %>%
...
data <- data %>%
  select(Nokapst,Nosjp,Jkpst,Umur,Nmdati2Layan,Nmppklayan,
         Klsrawat,Tglpelayanan,Tgldtgsjp,Tglplgsjp,Kdinacbgs,
         Nminacbgs,Diagprimer,Diagsekunder,Procedure,Namadpjp,
         Nmjnspulang,biayars,Biayaverifikasi,Keterangan)
write.xlsx(data, file = "bayi_randegansari.xlsx")

write.csv(datax, "D://data gresik//MTF KC GRESIK//gresik_2020//ur_bayi.csv",
          na="", row.names = FALSE)
#==================================================================
